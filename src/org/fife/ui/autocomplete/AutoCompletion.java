...
public class AutoCompletion {
  ...
  protected void insertCompletion(
    ...
    if (
      isParameterAssistanceEnabled() && (c instanceof ParameterizedCompletion)
    ) {
    ...
    if (pc.getParamCount() == 0 && !(pc instanceof TemplateCompletion)) {
      ...
      if (typedParamListStartChar) {
        ...
        textComponent.replaceSelection(
          Character.toString(p.getParameterListStart())
        );
  ...
  private class AutoActivationListener
    ...
    public void insertUpdate(DocumentEvent e) {
      ...
      if (
        ...
        if (
          textComponent != null && provider.isAutoActivateOkay(textComponent)
        ) {
          timer.restart();
          justInserted = true;
        } else {
