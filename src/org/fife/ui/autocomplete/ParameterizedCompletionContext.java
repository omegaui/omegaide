...
class ParameterizedCompletionContext {
  ...
  public void activate() {
    ...
    if (pc.getShowParameterToolTip()) {
      tip =
        new ParameterizedCompletionDescriptionToolTip(
          parentWindow,
          this,
          ac,
          pc
   ...
   */
  private Highlight replaceHighlightedText(
    Document doc,
    Highlight h,
    String replacement
  ) {
  ...
  private class ClosingAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (dot >= maxPos.getOffset() - 2) { // ">=" for overwrite mode
        ...
        //tc.setCaretPosition(maxPos.getOffset());
        tc.setCaretPosition(
          Math.min(tc.getCaretPosition() + 1, tc.getDocument().getLength())
        );
  ...
  private class Listener
    ...
    public void install(JTextComponent tc) {
      ...
      try {
        ...
        ParameterizedCompletionInsertionInfo info = pc.getInsertionInfo(
          tc,
          replaceTabs
        );
        ...
        for (int i = 0; i < replacementCount; i++) {
          ...
          // "-1" is a workaround for Java Highlight issues.
          tags.add(
            h.addHighlight(dr.getStartOffset() - 1, dr.getEndOffset(), painter)
          );
        }
