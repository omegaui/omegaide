...
public class TemplateCompletion
  ...
  private void parse(String template) {
    ...
    while (
      (offs = template.indexOf('$', lastOffs)) > -1 &&
      ...
      switch (next) {
        case '$': // "$$" => escaped single dollar sign
          addTemplatePiece(
            new TemplatePiece.Text(template.substring(lastOffs, offs + 1))
          );
          lastOffs = offs + 2;
          break;
        case '{': // "${...}" => variable
          ...
          if (closingCurly > -1) {
            addTemplatePiece(
              new TemplatePiece.Text(template.substring(lastOffs, offs))
            );
