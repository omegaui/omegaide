          ...
          case '\n':
            if (lastWasCR) {
              if (counter > (last + 1)) {
                doc.insertString(
                  pos,
                  new String(buff, last, counter - last - 1),
                  null
                );
                pos += (counter - last - 1);
              }
      ...
      try {
        ...
        // No line wrap - optimized for performance!
          ...
          if (count > 0) { // If there are chars in the line...
            ...
            firstNonWhitespace =
              currentLineStart + (firstNonWhitespace - currentLine.offset);
    ...
    public void actionPerformedImpl(ActionEvent e, RTextArea textArea) {
      ...
      try {
        ...
        if (offs < curPara.getStartOffset()) {
          offs =
            Utilities.getParagraphElement(textArea, offs).getEndOffset() - 1;
        }
      } catch (BadLocationException bl) {
        if (offs != 0) {
          offs = 0;
    ...
    public void actionPerformedImpl(ActionEvent e, RTextArea textArea) {
      ...
      if (selectedIndex != -1) {
        try {
          ...
          yOffset =
            direction * ((int) Math.ceil(scrollAmount / (double) h) - 1) * h;
          newVis.y =
            constrainY(textArea, initialY + yOffset, yOffset, visible.height);
          int newIndex;
          ...
          if (visible.contains(dotBounds.x, dotBounds.y)) {
            ...
            newIndex =
              textArea.viewToModel(
                new Point(x, constrainY(textArea, dotBounds.y + yOffset, 0, 0))
              );
          } else {
            ...
            if (direction == -1) {
              ...
              newIndex =
                textArea.viewToModel(new Point(x, newVis.y + visible.height));
            }
          }
