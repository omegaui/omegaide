...
public class RTextArea extends RTextAreaBase implements Printable {
  ...
  private JMenuItem clickImageMenuItem;
  ...
  protected void configurePopupMenu(JPopupMenu popupMenu) {
    ...
    clickImageMenuItem.addActionListener(e ->
      omega.Screen.getScreen().getCurrentEditor().saveImage()
    );
        ...
        try {
          for (MacroRecord record : macroRecords) {
            for (Action action : actions) {
              if (
                (action instanceof RecordableTextAction) &&
  ...
  public int print(Graphics g, PageFormat pageFormat, int pageIndex) {
    return RPrintUtilities.printDocumentWordWrap(
      g,
      this,
      getFont(),
      pageIndex,
  ...
  public void setCaret(Caret caret) {
    ...
    if (
      carets != null && // Called by setUI() before carets is initialized
  ...
  public void setDocument(Document document) {
    if (!(document instanceof RDocument)) {
      throw new IllegalArgumentException(
        "RTextArea requires " + "instances of RDocument for its document"
      );
    }
