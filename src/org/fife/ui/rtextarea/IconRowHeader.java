...
public class IconRowHeader
  ...
  public Dimension getPreferredSize() {
    ...
    return new Dimension(
      (textArea != null && textArea.getGraphics() != null)
        ? (width = textArea.getGraphics().getFontMetrics().getHeight() + 2)
        : width,
      h
    );
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (
      (activeLineRangeStart >= topLine && activeLineRangeStart <= bottomLine) ||
  ...
  private void paintComponentWrapped(Graphics g) {
    ...
    Rectangle r = IconRowHeader.getChildViewBounds(
      v,
      topLine,
      visibleEditorRect
    );
