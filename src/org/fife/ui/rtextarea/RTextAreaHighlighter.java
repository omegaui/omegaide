...
public class RTextAreaHighlighter extends BasicHighlighter {
  ...
  protected void paintListLayered(
    Graphics g,
    int lineStart,
    int lineEnd,
    Shape viewBounds,
    ...
    for (int i = highlights.size() - 1; i >= 0; i--) {
      ...
      if (tag instanceof LayeredHighlightInfo) {
        ...
        if (
          (lineStart < highlightStart && lineEnd > highlightStart) ||
          (lineStart >= highlightStart && lineStart < highlightEnd)
        ) {
          lhi.paintLayeredHighlights(
            g,
            lineStart,
            lineEnd,
            viewBounds,
  ...
  protected static class LayeredHighlightInfoImpl
    ...
    public void paintLayeredHighlights(
      ...
      p1 = Math.min(end, p1);
      if (
        getColor() != null && getPainter() instanceof ChangeableHighlightPainter
      ) {
      ...
      union(
        ((LayeredHighlighter.LayerPainter) getPainter()).paintLayer(
            g,
            p0,
            p1,
            viewBounds,
