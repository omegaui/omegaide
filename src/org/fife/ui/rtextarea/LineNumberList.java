...
*/
    ...
    if (ltr) {
      ...
      int line = topLine + 1;
      while (
        y < visibleRect.y + visibleRect.height + ascent &&
        line <= textArea.getLineCount()
      ) {
        String number = Integer.toString(
          line + getLineNumberingStartIndex() - 1
        );
        int width = metrics.stringWidth(number);
        if (
          getTextArea().getCaretLineNumber() +
          1 ==
          line +
          getLineNumberingStartIndex() -
          1
        ) g.setColor(omega.utils.UIManager.TOOLMENU_COLOR5); else g.setColor(
          getForeground()
        );
      ...
      int line = topLine + 1;
      while (
        y < visibleRect.y + visibleRect.height && line < textArea.getLineCount()
      ) {
        String number = Integer.toString(
          line + getLineNumberingStartIndex() - 1
        );
  ...
  private void paintWrappedLineNumbers(Graphics g, Rectangle visibleRect) {
    ...
    Rectangle r = LineNumberList.getChildViewBounds(
      v,
      topLine,
      visibleEditorRect
    );
  ...
  void updateCellWidths() {
    ...
    if (textArea != null) {
      ...
      if (font != null) {
        ...
        int count = 0;
        int lineCount =
          textArea.getLineCount() + getLineNumberingStartIndex() - 1;
        do {
  ...
  private class Listener implements CaretListener, PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent e) {
      ...
      if (
        RTextArea.HIGHLIGHT_CURRENT_LINE_PROPERTY.equals(name) ||
