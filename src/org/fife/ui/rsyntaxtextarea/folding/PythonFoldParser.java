...
public class PythonFoldParser implements FoldParser {
    ...
    try {
      for (int line = 0; line < lineCount; line++) {
        ...
        if (leadingWhiteSpaceCount == currentLeadingWhiteSpaceCount) {
          ...
          if (currentFold != null) {
            currentFold =
              currentFold.createChild(FoldType.CODE, currentNextFoldStart);
          } else {
            currentFold =
              new Fold(FoldType.CODE, textArea, currentNextFoldStart);
          ...
          while (
            !foldStartLeadingWhiteSpaceCounts.isEmpty() &&
  ...
  private static int getLeadingWhiteSpaceCount(Token t, int tabSize) {
    ...
    if (
      t == null ||
      t.getType() == TokenTypes.LITERAL_STRING_DOUBLE_QUOTE ||
