...
public class HtmlFoldParser implements FoldParser {
          ...
          if (language >= 0 && t.getType() == Token.SEPARATOR) {
            ...
            if (t.startsWith(LANG_START[language])) {
              if (currentFold == null) {
                ...
                currentFold =
                  currentFold.createChild(FoldType.CODE, t.getOffset());
              }
              inSublanguage = true;
            }
            // ?> or %>
          ...
          if (!inSublanguage) {
            if (t.getType() == Token.COMMENT_MULTILINE) {
              ...
              // Starting a MLC that ends on a later line...
                ...
                if (currentFold == null) {
                  currentFold =
                    new Fold(FoldType.COMMENT, textArea, t.getOffset());
                  folds.add(currentFold);
                } else {
                  currentFold =
                    currentFold.createChild(FoldType.COMMENT, t.getOffset());
                }
                inMLC = true;
              }
              // Starting a JSP comment that ends on a later line...
              else if (
                language == LANGUAGE_JSP &&
            ...
            // If we're starting a new tag...
              ...
              if (isFoldableTag(tagNameToken)) {
                ...
                if (
                  tagCloseToken.isSingleChar(Token.MARKUP_TAG_DELIMITER, '>')
                ) {
                  if (currentFold == null) {
                    currentFold =
                      new Fold(
            ...
            // If we've found a closing tag (e.g. "</div>").
              ...
              t.is(Token.MARKUP_TAG_DELIMITER, MARKUP_CLOSING_TAG_START)
            ) {
              if (currentFold != null) {
                Token tagNameToken = t.getNextToken();
                if (
                  isFoldableTag(tagNameToken) &&
   ...
   */
  private static boolean isEndOfLastFold(
  ...
  private static class TagCloseInfo {
    ...
    public String toString() {
      return (
        "[TagCloseInfo: " + "closeToken=" + closeToken + ", line=" + line + "]"
      );
    }
  }
}
