...
public class TokenImpl implements Token {
   ...
   */
  private StringBuilder appendHtmlLexeme(
    RSyntaxTextArea textArea,
    StringBuilder sb,
    boolean tabsToSpaces
  ) {
  ...
  public Rectangle listOffsetToView(
    ...
    while (token != null && token.isPaintable()) {
      ...
      // the bounding box...
        ...
        stableX +=
          Utilities.getTabbedTextWidth(s, fm, stableX, e, token.getOffset());
      }
  ...
  public void moveOffset(int amt) {
    if (amt < 0 || amt > textCount) {
      throw new IllegalArgumentException(
        "amt " + amt + " is not in range 0-" + textCount
      );
    }
