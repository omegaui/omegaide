...
public final class RSyntaxUtilities implements SwingConstants {
  ...
  public static Rectangle getLineWidthUpTo(
    ...
    TokenSubList subList = TokenUtils.getSubTokenList(
      t,
      p0,
      e,
      textArea,
   ...
   *         isn't on a bracket).
   */
  public static Point getMatchingBracketPosition(
    RSyntaxTextArea textArea,
    Point input
  ) {
    if (input == null) {
   ...
   */
  public static Token getNextImportantToken(
    Token t,
    RSyntaxTextArea textArea,
    int line
  ) {
          ...
          if (target.isCodeFoldingEnabled()) {
            ...
            if (last != current) { // If moving down a line...
              ...
              if (fm.isLineHidden(current)) {
                ...
                pos =
                  current == lineCount
                    ? target.getLineEndOffset(last) - 1
                    : target.getLineStartOffset(current); // Was the last visible line
              }
            }
          }
        }
   ...
   */
  public static int getPositionAbove(
    RSyntaxTextArea c,
    int offs,
    float x,
    TabExpander e
   ...
   * @exception BadLocationException if the offset is out of range
   */
  public static int getPositionBelow(
    RSyntaxTextArea c,
    int offs,
    float x,
    TabExpander e
   ...
   */
  public static float getTokenListWidth(
    Token tokenList,
    RSyntaxTextArea textArea,
    TabExpander e
  ) {
   ...
   */
  public static float getTokenListWidthUpTo(
    final Token tokenList,
    RSyntaxTextArea textArea,
    TabExpander e,
    float x0,
    ...
    for (
      ...
      if (t.containsPosition(upTo)) {
        return (
          width + t.getWidthUpTo(upTo - t.getOffset(), textArea, e, x0 + width)
        );
      }
  ...
  public static boolean regexCanFollowInJavaScript(Token t) {
    ...
    return (
      ...
      (
        ...
        (t.charAt(t.length() - 1) == '=' || t.is(JS_AND) || t.is(JS_OR))
      ) ||
   ...
   */
  public static void selectAndPossiblyCenter(
    JTextArea textArea,
    DocumentRange range,
    boolean select
  ) {
   ...
   * @return The pattern.
   */
  public static Pattern wildcardToPattern(
    String wildcard,
    boolean matchCase,
