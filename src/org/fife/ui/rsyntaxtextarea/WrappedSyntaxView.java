...
public class WrappedSyntaxView
  ...
  protected void drawView(
    ...
    while (token != null && token.isPaintable()) {
      ...
      while (
        token != null && token.isPaintable() && token.getEndOffset() - 1 < p
      ) { //<=p) {
    ...
    while (token != null && token.isPaintable()) {
      ...
      while (
        ...
        // This token is entirely selected
          ...
          token.getOffset() >= selStart && token.getEndOffset() <= selEnd
        ) {
          x = painter.paintSelected(token, g, x, y, host, this, useSTC);
        }
  ...
  public Shape modelToView(int pos, Shape a, Position.Bias b)
    ...
    if ((vIndex != -1) && (vIndex < getViewCount())) {
      ...
      if (
        v != null && testPos >= v.getStartOffset() && testPos < v.getEndOffset()
      ) {
  ...
  public void paint(Graphics g, Shape a) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (tempRect.intersects(clip)) {
        ...
        View view = getView(i);
        if (
          selStart == selEnd || startOffset >= selEnd || endOffset < selStart
        ) {
          drawView(
            painter,
            g2d,
