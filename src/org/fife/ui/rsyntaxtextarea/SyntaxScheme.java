...
public class SyntaxScheme implements Cloneable, TokenTypes {
  ...
  void changeBaseFont(Font old, Font font) {
    for (Style style : styles) {
      if (style != null && style.font != null) {
        if (
          style.font.getFamily().equals(old.getFamily()) &&
  ...
  public void restoreDefaults(Font baseFont, boolean fontStyles) {
    ...
    if (fontStyles) {
      ...
      StyleContext sc = StyleContext.getDefaultStyleContext();
      Font boldFont = sc.getFont(
        baseFont.getFamily(),
        Font.BOLD,
        baseFont.getSize()
      );
      Font italicFont = sc.getFont(
        baseFont.getFamily(),
        Font.ITALIC,
        baseFont.getSize()
      );
    ...
    styles[COMMENT_KEYWORD] =
      new Style(new Color(255, 152, 0), null, commentFont);
