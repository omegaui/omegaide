...
class ParserManager
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    int firstLine = firstOffsetModded == null
      ? 0
    ...
    if (DEBUG_PARSING) {
      System.out.println(
        "[DEBUG]: Minimum lines to parse: " + firstLine + "-" + lastLine
      );
    }
  ...
  private void addParserNoticeHighlights(ParseResult res) {
    ...
    if (DEBUG_PARSING) {
      System.out.println(
        "[DEBUG]: Adding parser notices from " + res.getParser()
      );
    }
      ...
      for (ParserNotice notice : notices) {
        ...
        try {
          ...
          if (notice.getShowInEditor()) {
            highlight =
              h.addParserHighlight(notice, parserErrorHighlightPainter);
          }
  ...
  public ToolTipInfo getToolTipText(MouseEvent e) {
    ...
    if (noticeHighlightPairs != null) {
      for (NoticeHighlightPair pair : noticeHighlightPairs) {
        ...
        if (
          noticeContainsPosition(notice, pos) &&
