...
public abstract class AbstractJFlexCTokenMaker extends AbstractJFlexTokenMaker {
  ...
  protected class CStyleInsertBreakAction
    ...
    public void actionPerformedImpl(ActionEvent e, RTextArea textArea) {
      ...
      if (
        type == Token.COMMENT_DOCUMENTATION || type == Token.COMMENT_MULTILINE
      ) {
    ...
    private void insertBreakInMLC(
      ...
      if (m.lookingAt()) {
        ...
        if (
          dot >= start && dot < start + leadingWS.length() + mlcMarker.length()
        ) {
          ...
          boolean moved = false;
          while (
            dot < end - 1 && Character.isWhitespace(text.charAt(dot - start))
          ) {
            moved = true;
            dot++;
          }
