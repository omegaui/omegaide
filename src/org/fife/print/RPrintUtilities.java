...
public abstract class RPrintUtilities {
  ...
  public static int printDocumentMonospaced(
    ...
    while (currentDocLineNumber < numDocLines) {
      ...
      try {
        curLineString =
          doc.getText(startOffs, currentLine.getEndOffset() - startOffs);
      ...
      if (tabSizeInSpaces == 0) {
        ...
        while (tabIndex > -1) {
          ...
          curLineString =
            curLineString.replaceFirst("\t", stringBuilder.toString());
      ...
      while (curLineString.length() > maxCharsPerLine) {
        ...
        curLineString =
          curLineString.substring(maxCharsPerLine, curLineString.length());
      }
   ...
   */
  public static int printDocumentMonospacedWordWrap(
    Graphics g,
    Document doc,
    int fontSize,
    int pageIndex,
    ...
    while (currentDocLineNumber < numDocLines) {
      ...
      while (curLineString.length() > maxCharsPerLine) {
        ...
        curLineString =
          curLineString.substring(breakPoint, curLineString.length());
      }
  ...
  public static int printDocumentWordWrap(
    ...
    final int maxLinesPerPage = (int) pageFormat.getImageableHeight() /
    fontHeight;
      ...
      // If it won't fit on a printed line by itself (i.e., it needs to be wrapped)...
        ...
        while (currentLineLengthInPixels > lineLengthInPixels) {
          ...
          for (char breakChar : BREAK_CHARS) {
            ...
            //	currentPos = pos;
            if (
              pos > 0 && pos > currentPos && pos != currentLineString.length()
            ) {
              currentPos = pos;
            }
          }
  ...
  private static Segment removeEndingWhitespace(Segment segment) {
    ...
    while (
      (currentChar == ' ' || currentChar == '\t') && currentChar != Segment.DONE
    ) {
      toTrim++;
