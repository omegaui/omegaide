...
public class TextComp extends JComponent {
    ...
    addMouseListener(
      new MouseAdapter() {
        ...
        public void mousePressed(MouseEvent e) {
          ...
          repaint();
          if (
            TextComp.this.runnable != null && e.getButton() == 1
    ...
    addMouseMotionListener(
      new MouseAdapter() {
        ...
        public void mouseDragged(MouseEvent e) {
          if (window != null) {
            window.setLocation(
              e.getXOnScreen() - pressX - getX(),
              e.getYOnScreen() - pressY - getY()
            );
          }
        }
      }
  ...
  public boolean isPaintGradientEnabled() {
    if (!paintGradientEnabled) return false;
    if (
      !(
        gradientMode >= GRADIENT_MODE_DEFAULT &&
        gradientMode <= GRADIENT_MODE_LINEAR
      )
    ) return false;
    if (gradientMode == GRADIENT_MODE_DEFAULT) return colorG != null;
    if (gradientMode == GRADIENT_MODE_LINEAR) return (
      fractions != null &&
      gradientColors != null &&
      fractions.length == gradientColors.length
    );
    return true;
  }
  ...
  public boolean isPaintTextGradientEnabled() {
    if (!paintTextGradientEnabled) return false;
    if (
      !(
        gradientMode >= GRADIENT_MODE_DEFAULT &&
        gradientMode <= GRADIENT_MODE_LINEAR
  ...
  public void addHighlightText(String... texts) {
    for (String text : texts) highlightTexts.add(text);
    texts = null;
    repaint();
  }
  ...
  public void paint(Graphics graphics) {
    ...
    if (isPaintGradientEnabled()) {
      if (gradientMode == GRADIENT_MODE_DEFAULT) g.setPaint(
        new GradientPaint(0, 0, color2, getWidth(), getHeight(), colorG)
      ); else if (gradientMode == GRADIENT_MODE_LINEAR) g.setPaint(
  ...
  public void draw(Graphics2D g, int x, int y) {
    ...
    if (isPaintTextGradientEnabled()) {
      if (gradientMode == GRADIENT_MODE_DEFAULT) g.setPaint(
        new GradientPaint(textX, y, color3, textWidth, textHeight, colorG)
      ); else if (gradientMode == GRADIENT_MODE_LINEAR) g.setPaint(
        new LinearGradientPaint(
          textX,
          y,
    ...
    for (String match : highlightTexts) {
      if (text.contains(match)) {
        ...
        if (matchLength <= text.length()) {
          ...
          g.drawString(
            match,
            textX + g.getFontMetrics().stringWidth(lpart),
            textY
          );
          g.setColor(colorH);
          g.drawString(
  ...
  public void paintEnter(Graphics2D g) {
    if (isPaintGradientEnabled() && !clickable) return;
    g.setColor(color1);
    g.fillRoundRect(0, 0, getWidth(), getHeight(), arcX, arcY);
  ...
  public void paintArc(Graphics2D g) {
    if (!topLeftArcVisible) g.fillRect(0, 0, arcX, arcY);
    if (!bottomLeftArcVisible) g.fillRect(0, getHeight() - arcY, arcX, arcY);
    if (!topRightArcVisible) g.fillRect(getWidth() - arcX, 0, arcX, arcY);
    if (!bottomRightArcVisible) g.fillRect(
      getWidth() - arcX,
      getHeight() - arcY,
      arcX,
      arcY
