...
public class NoCaretField
  ...
  public void paint(Graphics graphics) {
    ...
    if (x > BORDER_GAP) {
      ...
      if (lastText == null || lastText.equals("")) {
        ...
        while (x > BORDER_GAP && i < text.length()) {
          x =
            getWidth() / 2 - g.getFontMetrics().stringWidth(lastText) / 2 - 10;
          lastText += text.charAt(i++);
        }
      }
      String tempText =
        ".." + text.substring(text.length() - lastText.length() + 5);
    ...
    if (text == null || text.equals("")) {
      ...
      if (!isFocusOwner()) g.setColor(color3);
      g.fillRoundRect(
        getWidth() / 2 + g.getFontMetrics().stringWidth(lastText) / 2 - 1,
        getHeight() /
        2 -
        g.getFontMetrics().getHeight() /
        2 +
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (ctrl && v) {
      Transferable obj = (Transferable) Toolkit
        .getDefaultToolkit()
        .getSystemClipboard()
        .getContents(null);
      if (obj != null) {
        try {
          if (
            ...
            text + obj.getTransferData(DataFlavor.stringFlavor).toString()
          );
        } catch (Exception ex) {
          ex.printStackTrace();
        }
    ...
    if (editable) {
      if (Character.isLetterOrDigit(ch) || isSymbol(ch)) text += ch; else if (
        code == VK_BACK_SPACE
      ) {
        if (text.length() == 1) text = ""; else if (text.length() > 1) text =
  ...
  public void keyReleased(KeyEvent e) {
    int code = e.getKeyCode();
    if (code == VK_CONTROL) ctrl = false;
    if (code == VK_C) c = false;
    if (code == VK_V) v = false;
    if (code == VK_X) x = false;
  }
