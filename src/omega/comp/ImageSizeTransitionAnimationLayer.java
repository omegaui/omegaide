...
public abstract class ImageSizeTransitionAnimationLayer
  ...
  public synchronized void prepareImages(
    ...
    if (tempColor == color && image == comp.image) return;

    if (!useAddOn) images.clear();
    ...
    while ((positive ? (distance >= 0) : (distance < 0))) {
      ...
      g.drawImage(
        comp.image.getScaledInstance(sizeW, sizeH, Image.SCALE_SMOOTH),
        comp.getWidth() / 2 - sizeW / 2,
        comp.getHeight() / 2 - sizeH / 2,
        sizeW,
        sizeH,
        null
      );
