...
public class Startup extends JDialog {
      ...
      image =
        ImageIO.read(
          getClass()
            .getResourceAsStream(
              isDarkMode()
                ? "/omega_ide_icon64_dark.png"
                : "/omega_ide_icon64.png"
            )
        );
  ...
  public void init() {
    ...
    JScrollPane scrollPane = new JScrollPane(
      textArea = new RSyntaxTextArea(LICENSE_TEXT)
    );
    ...
    acceptComp =
      new TextComp(
        ...
        () -> {
          try {
            new File(".omega-ide" + File.separator + ".firststartup")
              .createNewFile();
            dispose();
          } catch (Exception e) {
            e.printStackTrace();
  ...
  public static void checkStartup(Screen screen) {
    if (!new File(".omega-ide" + File.separator + ".firststartup").exists()) {
      ...
      try {
        ...
        ) FlatDarkLaf.install(); else FlatLightLaf.install();
      } catch (Exception e) {
  ...
  public static void writeUIFiles() {
    ...
    if (!new File(".omega-ide", ".ui").exists()) {
      omega.utils.UIManager.loadDefaultFile(
        ".omega-ide" + File.separator + ".ui",
        ".omega-ide/.ui"
      );
    }
    if (!new File(".omega-ide", ".preferences").exists()) {
      omega.utils.UIManager.loadDefaultFile(
        ".omega-ide" + File.separator + ".preferences",
        ".omega-ide/.preferences"
      );
    }
    if (!new File(".omega-ide", ".snippets").exists()) {
      omega.utils.UIManager.loadDefaultFile(
        ".omega-ide" + File.separator + ".snippets",
        ".omega-ide/.snippets"
      );
    }
    if (!new File(".omega-ide", ".processExecutionData").exists()) {
      omega.utils.UIManager.loadDefaultFile(
        ".omega-ide" + File.separator + ".processExecutionData",
        ".omega-ide/.processExecutionData"
      );
    }
    f =
      new File(".omega-ide" + File.separator + "dictionary", "english_dic.zip");
    if (!f.exists()) {
      ...
      omega.utils.UIManager.loadDefaultFile(
        ...
        File.separator +
        "english_dic.zip",
        ".omega-ide/dictionary/english_dic.zip"
      );
    }
    ...
    f.mkdir();
    if (
      !PluginManager.PLUGINS_DIRECTORY.exists()
    ...
    if (!f.exists()) {
      ...
      loadDefaultFile(
        "linux" + File.separator + "x86" + File.separator + "libpty.so",
        ".omega-ide/pty4j-libs/linux/x86/libpty.so"
      );
      loadDefaultFile(
        "linux" + File.separator + "x86_64" + File.separator + "libpty.so",
        ".omega-ide/pty4j-libs/linux/x86_64/libpty.so"
      );
      loadDefaultFile(
        "macosx" + File.separator + "x86" + File.separator + "libpty.dylib",
        ".omega-ide/pty4j-libs/macosx/x86/libpty.dylib"
      );
      loadDefaultFile(
        "macosx" + File.separator + "x86_64" + File.separator + "libpty.dylib",
        ".omega-ide/pty4j-libs/macosx/x86_64/libpty.dylib"
      );
      loadDefaultFile(
        "x86" + File.separator + "libwinpty.dll",
        ".omega-ide/pty4j-libs/x86/libwinpty.dll"
      );
      loadDefaultFile(
        "x86" + File.separator + "winpty-agent.exe",
        ".omega-ide/pty4j-libs/x86/winpty-agent.exe"
      );
      loadDefaultFile(
        "win" + File.separator + "x86_64" + File.separator + "winpty.dll",
        ".omega-ide/pty4j-libs/win/x86_64/winpty.dll"
      );
      loadDefaultFile(
        "win" + File.separator + "x86_64" + File.separator + "winpty-agent.exe",
        ".omega-ide/pty4j-libs/win/x86_64/winpty-agent.exe"
      );
      loadDefaultFile(
        "win" +
        File.separator +
        ...
        ".omega-ide/pty4j-libs/win/x86_64/winpty-debugserver.exe"
      );

      System.out.println(
        "Writing Native Library for Terminal Emulation ... Done!"
      );
      ...
      try {
        ...
        System.err.println(
          "An Exception occured in generating the \".generated-pty-native-libs\" file."
        );
        System.err.println(
          "This usually means that you are running the IDE in a non-owned directory"
        );
        System.err.println(
          "due to this the system denied permission for creating the file, "
        );
        System.err.println(
          "try running the IDE in an owned directory (like your home folder). If still this exception is occuring then, "
        );
        System.err.println(
          "Please Open an issue with this log message on https://github.com/omegaui/omegaide"
        );
        System.err.println(e);
      }
