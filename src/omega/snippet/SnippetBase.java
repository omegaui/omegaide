...
public class SnippetBase {
  ...
  private static File dataBase = new File(
  ...
  public static void loadDefault() {
    try {
      Scanner reader = new Scanner(
        SnippetBase.class.getResourceAsStream("/.snippets")
      );
      ...
      while (reader.hasNextLine()) {
        ...
        if (line.startsWith(CARET)) {
          try {
            snip.caret =
              Integer.parseInt(
                line.substring(CARET.length(), line.indexOf(','))
              );
  ...
  public static void insertSnippet(
    JTextArea textArea,
    String base,
    int index,
    String tabs
      ...
      if (s.base.equals(base)) {
        ...
        while (tok.hasMoreTokens()) {
          ...
          if (c == 0) {
            ...
            if (tok.hasMoreTokens()) textArea.insert(
              "\n",
            ...
            if (tok.hasMoreTokens()) textArea.insert(
              tabs + line + "\n",
              textArea.getCaretPosition()
            ); else textArea.insert(tabs + line, textArea.getCaretPosition());
          }
          if (tok.hasMoreTokens()) c = 1;
        }
        if (s.line == 0) textArea.setCaretPosition(index + s.caret); else if (
          s.line > 0 && !tabs.equals("")
        ) textArea.setCaretPosition(
          index +
          s.caret +
          (analyzeTabs(tabs, s.line).length() - analyzeTabs(tabs, 0).length())
        ); else textArea.setCaretPosition(index + s.caret);
        return;
      }
    }
  }
