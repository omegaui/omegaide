...
public class IndentationFramework {
  ...
  public static void indent(RTextArea textArea) {
    ...
    for (String token : lines) {
      token = token.trim();
      lineN++;
      if (lineN <= caretLineNumber) caretPos += (tabs / textArea.getTabSize());

      if (count('{', token) == count('}', token)) containsEqual =
        true; else containsEqual = false;

      if (!containsEqual) tabs -= count('}', token);

      if (!needsExtraTab) textArea.append(getTabs(tabs) + token + "\n"); else {
      ...
      if (
        (token.startsWith("if") && !token.contains(";")) ||
        token.startsWith("while") ||
        token.startsWith("for") ||
        token.startsWith("else")
      ) {
        if (
          count('{', token) == 0 && count('(', token) == count(')', token)
        ) needsExtraTab = true;
      }
    }
  ...
  public static int count(char c, String line) {
    ...
    for (int i = 0; i < line.length(); i++) {
      ...
      if (!instr && "\"\'".contains(ch + "")) instr = true; else if (
        "\"\'".contains(ch + "")
      ) instr = false;
      if (!instr && ch == c) count++;
    }
