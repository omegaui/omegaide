...
public class CodeFramework {
  ...
  public static boolean genFullPathHints(String code) {
    ...
    if (!isSource(className)) {
      ...
      if (has(className)) reader = getReader(className); else reader =
        Screen.getFileView().getJDKManager().prepareReader(className);
      ...
      if (code.indexOf('.', ix + 1) >= 0) {
        ...
        for (int i = 0; i < code.length(); i++) {
          ...
          if (ch == '.' && blocks == -1) {
            ...
            index = i + 1;
            members.add(x.trim());
          } else if (ch == '(') blocks++; else if (ch == ')') blocks--;
        }
        boolean least = false;
        int in = 0;
        for (String member : members) {
          inner:for (DataMember d : dataMembers) {
            ...
            if (
              ...
              if (d.name.equals(name) && d.parameterCount == parameterCount) {
                least = true;
                if (has(d.type)) reader = getReader(d.type); else reader =
                  Screen.getFileView().getJDKManager().prepareReader(d.type);
        ...
        if (least) gen(dataMembers); else {
          Screen
            .getScreen()
            .getToolMenu()
            .setTask("Not at least one match found for " + code);
            ...
            if (
              ...
              if (d.name.equals(name) && d.parameterCount == parameterCount) {
                ...
                if (reader != null) {
                  ...
                  if (!isSource(pack)) {
                    if (pack != null) {
                      if (has(pack)) readerB = getReader(pack); else readerB =
                        Screen
                          .getFileView()
                          .getJDKManager()
                          .prepareReader(pack);
                    } else {
                      if (has(d.type)) readerB =
                        getReader(d.type); else readerB =
                        Screen
                          .getFileView()
                          .getJDKManager()
                          .prepareReader(d.type);
                    }
                  ...
                  if (has(d.type)) readerB = getReader(d.type); else readerB =
                    Screen.getFileView().getJDKManager().prepareReader(d.type);
                  reader = null;
                  dataMembers = readerB.dataMembers;
                }
              ...
              if (reader != null) {
                ...
                if (!isSource(pack)) {
                  if (pack != null) {
                    if (has(pack)) readerB = getReader(pack); else readerB =
                      Screen.getFileView().getJDKManager().prepareReader(pack);
                  } else {
                    if (has(d.type)) readerB = getReader(d.type); else readerB =
                      Screen
                        .getFileView()
  ...
  public static boolean genClassHints(String code) {
    ...
    if (!isSource(path)) {
      ...
      if (!isAvailable) return false;
      ByteReader reader = null;
      if (has(path)) reader = getReader(path); else reader =
        Screen.getFileView().getJDKManager().prepareReader(path);
      dataMembers = reader.getDataMembers("static");
      //Checking whether if there is data after className
  ...
  public static boolean genInstanceHints(String code) {
    ...
    if (dataMember == null) {
      ...
      if (reader.recordingInternal) {
        reader = new SourceReader(partialText);
        String classNameOfLastInternalReader = reader.internalReaders.getLast()
          .className;
        SourceReader i = null;
      ...
      for (SourceReader.Import im : f_reader.imports) {
        if (im.isStatic) {
          if (im.staticMember.equals(var)) {
            dataMember =
              new DataMember("si", "static", im.get(), im.staticMember, "");
            break;
          }
        }
      }
    ...
    if (dataMember.access.equals("si")) {
      ...
      if (type.contains(".")) {
        ...
        if (cType.equals(typeP)) type = cType; else type = null;
      } else {
  ...
  public static boolean genAllFullPathHints(String code) {
    ...
    if (!isSource(className)) {
      ...
      if (code.indexOf('.', ix + 1) >= 0) {
        ...
        if (least) gen(dataMembers); else System.out.println(
          "Not at least one match found for " + code
        );
      } else {
        //There are no members after class-path 's dot, so showing all hints
        gen(dataMembers);
  ...
  public static int getParameterCount(String line) {
    ...
    if (line.contains("(") && line.contains(")")) {
      ...
      if (!p.contains(",")) parameterCount = 1; else parameterCount =
        p.split(",").length;
    }
  ...
  public static boolean isUpperCaseHintType(DataMember d, String lCode) {
    ...
    for (char ch : d.name.toCharArray()) {
      if (Character.isUpperCase(ch)) upperCode += ch;
    }
  ...
  public static void gen(LinkedList<DataMember> dataMembers) {
    ...
    if (lCode != null) {
      dataMembers.forEach(d -> {
        if (d.name.contains(lCode) || isUpperCaseHintType(d, lCode)) mx.add(d);
      });
  ...
  public static void gen(LinkedList<DataMember> dataMembers, Editor editor) {
    ...
    editor.contentWindow.genView(
      dataMembers,
      omega.Screen.getScreen().getGraphics()
    );
  }
  ...
  public static String getContent(String className) {
    ...
    if (className.contains(".")) {
      ...
      while (tok.hasMoreTokens()) files.add(tok.nextToken());
      String path =
    ...
    for (Editor ex : Screen.getScreen().getTabPanel().getEditors()) {
      if (
        ex.currentFile.getAbsolutePath().equals(file.getAbsolutePath())
      ) return ex.getText();
    }
    try {
  ...
  public static String getCode(String text, int caret) {
    ...
    if (count(text, '(') > count(text, ')')) {
      ...
      if (
        closeParenthesisCount < openParenthesisCount &&
    ...
    if (text.startsWith("return ")) text =
      text.substring("return ".length()).trim();
    if (text.startsWith("continue ")) text =
      text.substring("continue ".length()).trim();
    if (text.startsWith("break ")) text =
      text.substring("break ".length()).trim();
    if (text.contains("this.")) text =
      text.substring(text.lastIndexOf("this.") + "this.".length()).trim();
  ...
  public static String getCodeDoNotEliminateDot(String text, int caret) {
    ...
    if (text.contains("@")) text =
      text.substring(text.lastIndexOf('@') + 1).trim();
    if (text.startsWith("new ")) text =
      text.substring(text.lastIndexOf("new ") + 3).trim();
    if (text.startsWith("return ")) text =
      text.substring("return ".length()).trim();
    if (text.startsWith("continue ")) text =
      text.substring("continue ".length()).trim();
  ...
  public static String getLastCode(String text, int caret) {
    if (text.contains(".")) text =
      text.substring(text.lastIndexOf('.') + 1, caret);
  ...
  public static int getUnpairedParanthesis(String text) {
    ...
    for (int i = 0; i < text.length() - 1; i++) {
      ...
      if (c1 == '(' && Character.isLetterOrDigit(c2)) index = i + 1;
    }
    return index;
  }
