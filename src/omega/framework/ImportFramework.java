...
public class ImportFramework {
  ...
  public static LinkedList<String> findClasses(String text) {
    ...
    for (int i = 0; i < text.length(); i++) {
      ...
      if (!"\"\'_S".contains(ch + "") && !Character.isLetterOrDigit(ch)) {
        ...
        if (sx.equals("")) continue;
        if (text.charAt(i - sx.length()) != '.') {
          pos = i + 1;
          if (
            sx.length() > 0 &&
            Character.isUpperCase(sx.charAt(0)) &&
            !SourceReader.isInnerLine(sx) &&
            !cls.contains(sx)
          ) {
            cls.add(sx);
          }
   ...
   */
  public static synchronized void addImports(
    LinkedList<String> classes,
    Editor editor
  ) {
    if (JDKManager.reading) return;
    Screen.setStatus(
      "Resolving Imports ... Keep Editing meanwhile!",
      0,
    ...
    main:for (String classX : classes) {
      for (Import xm : JDKManager.javaLangPack) {
        if (xm.getClassName().equals(classX)) continue main;
      }
      if (reader.isSubClass(classX)) continue main;
    ...
    main:for (String classX : unimported) {
      LinkedList<String> bases = new LinkedList<>();
      inner:for (Import im : JDKManager.getAllImports()) {
        if (im.getClassName().equals(classX)) {
          if (
            im.getPackage().equals(PACK) ||
            contains(editor, im.getPackage(), im.getClassName()) ||
            CodeFramework.isSource(PACK + "." + im.getClassName())
          ) continue main;
          try {
      ...
      if (!bases.isEmpty()) coexistingClassess.add(bases);
    }
    ...
    for (LinkedList<String> bases : coexistingClassess) {
      ...
      for (int i = 0; i < bases.size(); i++) {
        for (int j = 0; j < bases.size() - i - 1; j++) {
          if (bases.get(j).equals(bases.get(j + 1))) bases.remove(j);
        }
      }
    }
    ...
    main:for (LinkedList<String> bases : coexistingClassess) {
      for (String pack : bases) {
        String base = pack.substring(0, pack.lastIndexOf('.'));
        if (
          base.equals(PACK) ||
          contains(editor, base, pack.substring(pack.lastIndexOf('.') + 1))
        ) {
          bases.clear();
          continue main;
        }
        if (bases.size() == 1) {
          ...
          outer:for (String classX : bases) {
            ...
            for (Import x : multiBases) {
              if (x.getImport().equals(im.getImport())) continue outer;
            }
            multiBases.add(im);
          }
        }
  ...
  public static synchronized int getPackageInformationIndex(Editor e) {
    int index = 0;
    LinkedList<String> lines = CodeTokenizer.tokenizeWithoutLoss(
      e.getText(),
      '\n',
      "package "
    );
   ...
   * @param className = The name of the class
   */
  public static void insertImport(
    Editor editor,
    String pack,
    String className
  ) {
    try {
      if (isPackageInformationPresent(editor)) {
        ...
        String insertedText = "";
        if (DataManager.isUsingStarImports()) d.insertString(
          start,
          insertedText =
            (!info.startChar.equals("") ? info.startChar : "") +
            "import " +
            pack +
            ".*;" +
            (!info.endChar.equals("") ? info.endChar : ""),
          null
        ); else d.insertString(
          start,
          insertedText =
      ...
      editor
        ...
        .setValue(editor.getAttachment().getVerticalScrollBar().getValue() - 1);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
   ...
   *
   */
  public static IndexInfo getInsertIndex(
    Editor editor,
    String pack,
    String className
  ) {
    ...
    try {
      while (
        line.endsWith(";") || line.trim().equals("") || !sourceStartReached
      ) {
        sourceStartReached = line.contains("public ");
        if (sourceStartReached) break;

        if (line.trim().equals("")) {
          line =
            text
              .substring(lineIndex + 1, text.indexOf(';', lineIndex) + 1)
              .trim();
          continue;
        }

        line = line.substring(line.indexOf(' '), line.indexOf(';')).trim();
        if (line.startsWith("static ")) line =
