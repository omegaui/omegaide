...
public class JDKManager {
  ...
  public JDKManager(File jdkDir) {
    ...
    try {
      if (isJDKPathValid(jdkDir.getAbsolutePath())) {
        loadVersionInfo();
        if (isModularJDK()) readModules(); else readRTJarFile();
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public void readRTJarFile() {
    ...
    try {
      ...
      try (JarFile rtJarFile = new JarFile(rtJarPath)) {
        for (
          Enumeration<JarEntry> enums = rtJarFile.entries();
      ...
      Screen.setStatus(
        "Exception while Reading JDK v" + version,
        99,
  ...
  public void readModules() {
    File[] modulesFiles = new File(
      jdkDir.getAbsolutePath() + File.separator + "jmods"
    )
      .listFiles();
    if (modulesFiles == null || modulesFiles.length == 0) return;
    for (File moduleFile : modulesFiles) modules.add(new Module(moduleFile));
  ...
  public void addModule(File moduleFile) {
    for (Module module : modules) {
      if (
        module.moduleFile.getAbsolutePath().equals(moduleFile.getAbsolutePath())
      ) return;
    }
  ...
  public void loadVersionInfo() {
    try {
      Scanner reader = new Scanner(
        new File(jdkDir.getAbsolutePath() + File.separator + "release")
      );
  ...
  public synchronized void readSources(String projectPath) {
    ...
    sourceFiles.forEach(source -> {
      ...
      sourcePath =
        sourcePath.substring(sourcePath.indexOf("src") + "src".length() + 1);
  ...
  public void readResources(
    String projectPath,
    LinkedList<String> resourceRoots
  ) {
    ...
    resourceRoots.forEach(resourceDir -> {
      ...
      if (!byteCodeFiles.isEmpty()) {
        byteCodeFiles.forEach(file -> {
          ...
          String qualifiedPath = path.substring(
            resourceDir.length() + 1,
            path.lastIndexOf(".")
          );
          String className = CodeFramework.replace(
            qualifiedPath,
            File.separatorChar,
            '.'
          );
  ...
  public synchronized String checkInResourceRoots(
    String pack,
    String className
  ) {
    for (String name : resourceCodeLoader.classNames) {
      if (
        name.startsWith(pack + ".") && name.endsWith("." + className)
      ) return name;
    }
    return null;
  }
  ...
  public void readJar(String path, boolean module) {
    Screen.setStatus(
      "Reading Jar : " + new File(path).getName(),
      10,
      IconManager.fluentjavaImage
    );
    try {
      ...
      Screen.setStatus(
        "Exception while Reading Jar : " + new File(path).getName(),
        12,
        IconManager.fluentbrokenbotImage
      );
      e.printStackTrace();
  ...
  public static int calculateVersion(File jdkDir) {
    ...
    version =
      version.substring(version.indexOf('\"') + 1, version.lastIndexOf('\"'));
    if (version.startsWith("1.")) return Integer.parseInt(
      version.charAt(2) + ""
    ); else if (version.contains(".")) return Integer.parseInt(
      version.substring(0, version.indexOf('.'))
    ); else return Integer.parseInt(version);
  }
  ...
  public void loadFiles(File dir, LinkedList<File> files, String ext) {
    ...
    for (File f : F) {
      if (f.isDirectory()) loadFiles(f, files, ext); else if (
  ...
  public void loadFiles(
    ...
    for (File f : F) {
      if (f.isDirectory()) loadFiles(f, files, ext); else if (
        f.getName().endsWith(ext) && condition.accept(f)
      ) files.add(f);
    }
  }
  ...
  public void addImport(String packagePath, String jarPath, boolean module) {
    ...
    if (im.packageName.equals("java.lang")) javaLangPack.add(im);
  }
  ...
  public void clear() {
    ...
    javaLangPack.clear();
    sources.clear();
    if (dependencyJarLoader != null) dependencyJarLoader.close();
    if (resourceCodeLoader != null) resourceCodeLoader.close();
  }
  ...
  public static synchronized boolean isSourceClass(String className) {
    for (Import im : sources) {
      if (im.className.equals(className)) return true;
    }
    return false;
  }
}
