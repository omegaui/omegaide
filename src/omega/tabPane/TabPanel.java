...
public class TabPanel extends JPanel {
  ...
  private static final String HINT1 =
    "Open the File tree by clicking the fourth ";
  private static final String HINT1_2 =
    "Open the File tree by clicking the second ";
  ...
  public void addTab(
    ...
    TabHolder panel = (component instanceof Editor)
      ? new EditorTabHolder((Editor) component)
      : new TabHolder(component);
      ...
      if (editor.currentFile == null) {
        tabPane.setTabComponentAt(
          ...
          TabComp.create(
            ...
            toolTip,
            isEditor ? createMenu((Editor) component) : null
          )
        );
        return;
      }
      ...
      if (editor.currentFile == null) {
        tabPane.setTabComponentAt(
          ...
          TabComp.create(
            ...
            toolTip,
            isEditor ? createMenu((Editor) component) : popup
          )
        );
        return;
      }
  ...
  public boolean contains(JComponent comp) {
    for (TabHolder holder : tabHolders) {
      if (holder.component == comp) return true;
    }
    return false;
  }
  ...
  public static OPopupWindow createMenu(Editor editor) {
    if (editor.currentFile != null) {
      ...
      ) return PopupManager.createPopup(
        PopupManager.SOURCE_FILE,
        editor,
        screen
      ); else return PopupManager.createPopup(
        PopupManager.NON_SOURCE_FILE,
        editor,
        screen
      );
    }
    ...
    for (Editor e : editors) {
      if (e != null) {
        if (e.currentFile != null) {
          if (
            e.currentFile.getAbsolutePath().equals(file.getAbsolutePath())
          ) return e;
        }
      }
    }
  ...
  public void remove(String name) {
    ...
    if (tabHolders.get(names.indexOf(name)) instanceof EditorTabHolder) {
      (
        (Editor) (tabHolders.get(names.indexOf(name)).component)
      ).saveCurrentFile();
      ((Editor) (tabHolders.get(names.indexOf(name)).component)).currentFile =
        null;
      editors.remove(
        ((Editor) (tabHolders.get(names.indexOf(name)).component))
      );
  ...
  public Editor getCurrentEditor() {
    if (
      tabHolders.get(tabPane.getSelectedIndex()) instanceof EditorTabHolder
  ...
  public void paint(Graphics graphics) {
    if (tabPane.getTabCount() == 0) {
      ...
      String hint = screen.getToolMenu().hidden
        ? (
          (
              Screen.getFileView().getProjectManager() != null &&
