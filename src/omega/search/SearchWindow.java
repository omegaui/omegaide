...
public class SearchWindow extends JDialog {
  ...
  public SearchWindow(Screen f) {
          ...
          if (!searchComps.isEmpty()) {
            if (e.getKeyCode() == KeyEvent.VK_UP && pointer > 0) {
              ...
              scrollPane
                ...
                .setValue(scrollPane.getVerticalScrollBar().getValue() - 50);
            } else if (
              e.getKeyCode() == KeyEvent.VK_DOWN &&
              pointer + 1 < searchComps.size()
            ) {
              searchComps.get(pointer).set(false);
              searchComps.get(++pointer).set(true);
              scrollPane
    ...
    TextComp reloadComp = new TextComp(
      "#",
      "Click to Reload File Tree",
  ...
  public void list(String text) {
    ...
    panel.setPreferredSize(
      new Dimension(scrollPane.getWidth() - 5, blocks + 50)
    );
    scrollPane.repaint();
  ...
  public void load(File f) {
    ...
    for (File file : files) {
      if (file.isDirectory()) load(file); else if (
