...
public class FileTreeBranch extends JComponent {
    ...
    addMouseListener(
      new MouseAdapter() {
        ...
        public void mouseEntered(MouseEvent e) {
          if (mode == EDIT_FILE_NAME_MODE) return;
          enter = true;
          repaint();
        }
        ...
        public void mousePressed(MouseEvent e) {
          ...
          if (e.getButton() == 1 && e.getClickCount() == 2) {
            ...
            if (popupMenu == null) {
              popupMenu =
                OPopupWindow
                  .gen("Tree Menu", Screen.getScreen(), 0, false)
                  .width(250);
  ...
  public void paintComponent(Graphics graphics) {
    ...
    if (isFocusOwner()) {
      g.fillRect(
        30,
        getHeight() - 3,
        g.getFontMetrics().stringWidth(displayName),
        2
      );
    }
    if (enter) {
  ...
  public void performRename(ActionEvent e) {
    ...
    FileOperationManager.silentMoveFile(
      file,
      new File(file.getParentFile().getAbsolutePath(), e.getActionCommand())
    );
  ...
  public boolean isParentOf(FileTreeBranch branch) {
    return branch
      .getFile()
      .getAbsolutePath()
      .startsWith(getFile().getAbsolutePath());
  }
  ...
  public static BufferedImage getPreferredImageForFile(File file) {
    if (file.isDirectory()) {
      ...
      if (files != null && files.length != 0) {
        for (File fx : files) {
          if (
            fx.getName().equals(".projectInfo")
          ) return IconManager.fluentfolderImage;
          if (
            fx.getName().startsWith("build.gradle")
          ) return IconManager.fluentmoduleImage;
        }
      } else if (files.length == 0) return IconManager.fluentemptyBoxImage;
      if (
        file
          .getAbsolutePath()
          .equals(
            Screen.getFileView().getProjectPath() + File.separator + "src"
          )
      ) return IconManager.fluentsourceImage; else if (
        file
          .getAbsolutePath()
          .equals(
            Screen.getFileView().getProjectPath() + File.separator + "bin"
          )
      ) return IconManager.fluentbinaryImage; else if (
        file
          .getAbsolutePath()
          .equals(
            Screen.getFileView().getProjectPath() + File.separator + "res"
          )
      ) return IconManager.fluentresourceImage; else if (
        file
          .getAbsolutePath()
          .equals(
            Screen.getFileView().getProjectPath() + File.separator + "out"
          )
      ) return IconManager.fluentoutImage; else if (
        file
          .getAbsolutePath()
          .equals(
            Screen.getFileView().getProjectPath() + File.separator + "lib"
          )
      ) return IconManager.fluentpowerImage; else if (
        file
          .getAbsolutePath()
          .startsWith(
            Screen.getFileView().getProjectPath() + File.separator + "src"
    ...
    if (file.getName().contains(".")) {
      ...
      if (ext.equals(".java")) return IconManager.fluentjavaImage;
      if (ext.equals(".py")) return IconManager.fluentpythonImage;
      if (ext.equals(".kt")) return IconManager.fluentkotlinImage;
      if (ext.equals(".groovy")) return IconManager.fluentgroovyImage;
      if (ext.equals(".dart")) return IconManager.fluentdartImage;
      if (
        ext.equals(".js") ||
        ext.equals(".css") ||
        ext.equals(".html") ||
      ...
      if (ext.equals(".c")) return IconManager.fluentcImage;
      if (ext.equals(".cpp")) return IconManager.fluentcplusplusImage;
      if (ext.equals(".rs")) return IconManager.fluentfileImage;
      if (
        ext.equals(".js") ||
        ext.equals(".html") ||
        ext.equals(".php") ||
      ...
      if (
        ext.equalsIgnoreCase(".appimage") || ext.equals(".deb")
      ) return IconManager.fluentlinuxImage;
      if (
        ext.equalsIgnoreCase(".fxml") || ext.equals(".xml")
      ...
      if (ext.equals(".dmg")) return IconManager.fluentmacImage;
      if (
  ...
  public static Color getPreferredColorForFile(File file) {
    if (file.isDirectory()) {
      if (file.getName().charAt(0) == '.') return TOOLMENU_COLOR4;
      return glow;
    }
    Color res = ANY_COLOR;
    if (!file.getName().contains(".")) return res;
    if (
      file.getName().endsWith(".java") ||
      file.getName().endsWith(".rs") ||
      ...
      file.getName().endsWith(".exe") || file.getName().endsWith(".msi")
    ) {
      ...
      file.getName().endsWith(".deb") ||
      file.getName().endsWith(".run") ||
      file.getName().endsWith(".sh")
    ) {
      res = LINUX_COLOR;
    } else if (
      file.getName().endsWith(".dependencies") ||
      file.getName().endsWith(".sources") ||
      file.getName().endsWith(".natives") ||
      file.getName().endsWith(".resources") ||
      file.getName().endsWith(".projectInfo") ||
      file.getName().endsWith(".modules") ||
      file.getName().endsWith(".snippets") ||
      file.getName().endsWith(".args")
    ) {
      res = UIManager.TOOLMENU_COLOR1;
      ...
      file.getName().endsWith(".xml") || file.getName().endsWith(".fxml")
    ) {
      res = XML_COLOR;
