...
public class RemotePluginView extends JDialog {
  ...
  public void init() {
    iconComp =
      new TextComp(remotePluginComp.iconComp.image, 30, 30, c2, c2, c2, null);
    ...
    versionComp =
      new TextComp(
        "Version : " + remotePluginInfo.version,
        back1,
        back1,
        TOOLMENU_COLOR1,
        null
    ...
    authorComp =
      new TextComp(
        "Developed By " + remotePluginInfo.author,
        back1,
        back1,
        TOOLMENU_COLOR3,
        null
    ...
    licenseComp =
      new TextComp(
        "Licensed Under " + remotePluginInfo.license,
        back1,
        back1,
        TOOLMENU_COLOR1,
        null
    ...
    leftComp =
      new TextComp("<", TOOLMENU_COLOR2, TOOLMENU_COLOR3, c2, this::moveLeft);
    ...
    rightComp =
      new TextComp(">", TOOLMENU_COLOR2, TOOLMENU_COLOR3, c2, this::moveRight);
    rightComp.setBounds(
      getWidth() - 2 - 30,
      160 + (getHeight() - 160) / 2 - 30 / 2,
      30,
      30
    );
    ...
    screenshotComp =
      new TextComp(
        remotePluginInfo.screenshotsURLs.isEmpty()
    ...
    screenshotComp.setBounds(
      35,
      160 + (getHeight() - 160) / 2 - (getHeight() - 200) / 2,
      getWidth() - 70,
      getHeight() - 200
    );
  ...
  public void moveRight() {
    if (pointer + 1 < screenshots.size()) pointer++;
    screenshotComp.image = screenshots.get(pointer);
    screenshotComp.repaint();
  }

  public void doPostInit() {
    if (remotePluginInfo.screenshotsURLs.isEmpty()) return;
      ...
      try {
        for (int i = 0; i < remotePluginInfo.screenshotsURLs.size(); i++) {
          descriptionComp.setText(
            ...
            (i + 1) +
            " of " +
            remotePluginInfo.screenshotsURLs.size()
          );
          screenshots.add(
            ImageIO.read(remotePluginInfo.screenshotsURLs.get(i))
          );
        }
      } catch (Exception e) {
        e.printStackTrace();
