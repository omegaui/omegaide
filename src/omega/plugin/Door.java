...
public class Door extends JComponent{
	...
	private volatile boolean enter;
     ...
     public Door(Plug plug, String path, BufferedImage image, Runnable r){
		...
		else omega.utils.UIManager.setData(this);
		setFont(FONT);
		addMouseListener(new MouseAdapter(){
			@Override
			public void mouseClicked(MouseEvent e){
                    if(omega.plugin.PluginView.plugin.getName().equals(plug.name)) {
                         plug.enabled = !plug.enabled;
                         if(plug.enabled) {
                        	     omega.Screen.getPluginManager().initPlug(plug.name);
                         }
                         else
                        	     omega.plugin.PluginView.plugin.disable();
                    }
                    r.run();
                    repaint();
			}
