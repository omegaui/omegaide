...
public class LocalPluginComp extends FlexPanel {
  ...
  public void init() {
    ...
    nameComp =
      new TextComp(
        ...
        c2,
        c2,
        PluginCategory.getSuitableColor(plugin.getPluginCategory()),
        null
      );
    ...
    versionComp =
      new TextComp(plugin.getVersion(), c2, c2, TOOLMENU_COLOR5, null);
    ...
    enableComp =
      new TextComp(
        pluginsView.pluginManager.isPluginEnabled(plugin)
          ? "Disable"
          : "Enable",
  ...
  public void toggleEnable() {
    pluginsView.pluginManager.put(
      plugin,
      !pluginsView.pluginManager.isPluginEnabled(plugin)
    );
    enableComp.setText(
      pluginsView.pluginManager.isPluginEnabled(plugin) ? "Disable" : "Enable"
    );
    if (
      !pluginsView.pluginManager.isPluginEnabled(plugin) &&
      plugin.needsRestart()
    ) {
      pluginsView.setStatus(
        "A Restart is required for disabling " + plugin.getName() + " Entirely!"
      );
    }
  }
