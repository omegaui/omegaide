...
public class SourceReader {
  ...
  public class Import {
    ...
    public Import(String pack, String name) {
      ...
      if (pack.startsWith("static ")) {
        ...
        pack =
          pack.substring(pack.indexOf(' ') + 1, pack.lastIndexOf('.')).trim();
      }
      this.pack = pack;
    }
  ...
  public void readImports() {
    ...
    for (String line : tokens) {
      ...
      if (line.trim().length() <= 1 && !line.trim().equals("}")) continue;
          ...
          if (line.contains(" static ")) {
            if (CodeFramework.isSource(pack)) {
              ...
              for (DataMember m : reader.getDataMembers("static")) {
                String nameX = m.name;
                if (nameX.contains("()")) nameX =
                  nameX.substring(0, nameX.indexOf('(')).trim();
                imports.add(new Import(pack, nameX));
              }
            } else {
              ByteReader reader = omega.Screen
                .getFileView()
                .getJDKManager()
                .prepareReader(pack);
              for (DataMember m : reader.getDataMembers("static")) {
      ...
      if (className == null) {
        String type = "";
        if (line.contains("class ")) type = "class"; else if (
          line.contains("enum ")
        ) type = "enum"; else if (line.contains("interface ")) type =
          "interface"; else if (line.contains("record ")) type = "record";
        ...
        if (!type.equals("")) {
          ...
          if (line.contains("{")) indexAfter = line.indexOf("{");
          ...
          if (line.contains("extends")) {
            ...
            if (line.contains("implements")) {
              ...
              if (x.contains("{")) x = x.substring(0, x.indexOf('{')); else if (
                x.endsWith("\n")
              ) x = x.substring(0, x.indexOf('\n'));
              if (!x.contains(",")) {
                features = new String[1];
                features[0] = x;
              } else features = x.split(",");
              for (int ix = 0; ix < features.length; ix++) features[ix] =
                features[ix].trim();
            } else {
              if (line.contains("{")) indexAfter =
                line.indexOf('{'); else indexAfter = line.indexOf('\n');
              parent = line.substring(extendsI + 1, indexAfter).trim();
            }
          }
  ...
  public void read() {
    ...
    for (int i = 0; i < tokens.size(); i++) {
      ...
      if (line.startsWith("import ") && line.endsWith(";") && canReadImports) {
        String im = line
          .substring(line.lastIndexOf(' ') + 1, line.indexOf(';'))
          .trim();
        String pack = im.substring(0, im.lastIndexOf('.'));
        String name = im.substring(im.lastIndexOf('.') + 1);
        if (
          pack.equals("java.lang") || !Character.isLowerCase(pack.charAt(0))
        ) continue;
        if (name.equals("*") && !JDKManager.reading) {
          if (line.contains(" static ")) {
            if (CodeFramework.isSource(pack)) {
              SourceReader reader = new SourceReader(
                CodeFramework.getContent(pack)
              );
              for (DataMember m : reader.getDataMembers("static")) {
                String nameX = m.name;
                if (nameX.contains("()")) nameX =
          ...
          imports.add(
            new Import(pack, name).setStatic(line.contains(" static "))
          );
        }
        continue;
      }
      ...
      if (className == null) {
        ...
        if (type.equals("record")) {
          ...
          for (String member : members) {
            ...
            dataMembers.add(
              new DataMember("private", "final", typeX, name, null, lineN)
            );
            dataMembers.add(
              new DataMember("public", "final", typeX, name + "()", "", lineN)
            );
          }
          members = null;
        } else if (!type.equals("")) {
      ...
      if (line.contains(" ") && !isInnerLine(line)) {
        ...
        if (cL.contains("(") && openBracesCount <= 1) {
          try {
            String parameters = cL
              .substring(cL.indexOf('(') + 1, cL.indexOf(')'))
              .trim();
          ...
          if (cL.contains(" ")) {
            ...
            if (cL.contains(" ")) {
              ...
              if (type != null) {
                dataMembers.add(
                  new DataMember(access, mods, type, name, null, lineN)
                );
                ownedDataMembers.add(dataMembers.getLast());
              }
            } else {
              ...
              if (type != null) {
                dataMembers.add(
                  new DataMember(access, "", type, name, null, lineN)
                );
                ownedDataMembers.add(dataMembers.getLast());
              }
            }
    ...
    if (!parent.contains(".")) parent = getPackage(parent);
    if (parent == null) return;
    if (!CodeFramework.isSource(parent)) {
      ByteReader byteReader = null;
      if (Assembly.has(parent)) byteReader =
        Assembly.getReader(parent); else byteReader =
        omega.Screen.getFileView().getJDKManager().prepareReader(parent);
    ...
    if (features != null) {
      for (String f : features) {
        if (!f.contains(".")) f = getPackage(f);
        if (!CodeFramework.isSource(f)) {
          ByteReader byteReader = null;
          if (Assembly.has(f)) byteReader =
            Assembly.getReader(f); else byteReader =
            omega.Screen.getFileView().getJDKManager().prepareReader(f);
          byteReader.dataMembers.forEach(this::offer);
        } else {
  ...
  public void offer(DataMember d) {
    for (DataMember dx : dataMembers) {
      if (
        dx.name.equals(d.name) &&
        dx.parameterCount == d.parameterCount &&
        dx.type.equals(d.type)
      ) {
        return;
      }
    }
  ...
  public static boolean isInnerLine(String line) {
    try {
      return (
        ...
        (
          line.contains("if") &&
          !Character.isLetter(line.charAt(line.indexOf("if") + 2))
        ) ||
        (
          line.contains("for") &&
          !Character.isLetter(line.charAt(line.indexOf("for") + 3))
        ) ||
        (
          line.contains("do") &&
          !Character.isLetter(line.charAt(line.indexOf("do") + 2))
        ) ||
        (
      ...
      try {
        return (
          ...
          (
            line.contains("if") &&
            !Character.isLetter(line.charAt(line.indexOf("if") - 1))
          ) ||
          (
            line.contains("for") &&
            !Character.isLetter(line.charAt(line.indexOf("for") - 1))
          ) ||
          line.contains("do") ||
          (
            line.contains("switch") &&
  ...
  public LinkedList<DepthMember> getMembers(DataMember m, String code) {
    ...
    for (DataBlock b : dataBlocks) {
      if (
        b.block.name.equals(m.name) &&
        b.block.parameters != null &&
        b.block.parameterCount == m.parameterCount
  ...
  public boolean isInternalReader(String className) {
    for (SourceReader r : internalReaders) {
      if (r.className.equals(className)) return true;
    }
    return false;
  }
  ...
  public String evaluateType(String type) {
    if (
      type != null &&
      !type.equals("") &&
      " var byte short int float double boolean long char void ".contains(type)
    ) return type;
  ...
  public String getPackage(String className) {
    ...
    if (CodeFramework.isSource(pack + "." + className)) return (
      pack + "." + className
    );
    for (SourceReader r : internalReaders) {
      if (r.className.equals(className)) return (
        pack + "." + this.className + "." + r.className
      );
    }
    try {
      className =
        Screen
          .getFileView()
          .getJDKManager()
          .checkInResourceRoots(pack, className);
    } catch (Exception e) {}
    return className;
  }
  ...
  public LinkedList<DataMember> getConstructors() {
    ...
    dataMembers.forEach(d -> {
      if (d.parameters != null) {
        if (
          d.name.equals(className + "()") && d.type.equals("")
        ) constructors.add(d);
      }
    });
  ...
  public boolean isSubClass(String className) {
    ...
    if (!path.equals("java.lang.Object")) {
      if (CodeFramework.isSource(path)) {
        value =
          new SourceReader(CodeFramework.getContent(path))
