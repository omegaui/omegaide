...
public class ContentTokenizer {
  ...
  public static boolean isObjectInstantiationCode(String code) {
    ...
    if (hasSpace && hasEqual) hasEqualsAfterSpace =
      code.indexOf(' ') < code.indexOf('=');
    if (hasEqual) hasEqualAtLast = code.charAt(code.length() - 1) == '=';
  ...
  public static boolean genConditionalHints(Editor e, String code) {
    if (isObjectInstantiationCode(code)) {
      ...
      if (im == null || className.equals("var")) return false;
      LinkedList<DataMember> dataMembers = new LinkedList<>();
      dataMembers.addFirst(
        new DataMember(
          "",
          "",
      ...
      reader.dataMembers.forEach(dataMember -> {
        ...
        if (type.contains(className)) {
          if (type.contains(".")) type =
            type.substring(type.lastIndexOf('.') + 1).trim();
          if (type.equals(className.trim())) dataMembers.add(dataMember);
        }
      });
  ...
  public static void arrangeTokens(Editor e, String text) {
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (
        token.equals("") || !token.contains(text) || token.equals(text)
      ) continue;
    ...
    main:for (DataMember token : tokens) {
      for (DataMember dx : dataMembers) {
        if (dx.name.equals(token.name)) continue main;
      }
    ...
    SnippetBase
      ...
      .forEach(snippet -> {
        dataMembers.add(
          new DataMember("", "", "Snippet", snippet.toString(), null)
        );
      });

    if (!dataMembers.isEmpty()) CodeFramework.gen(
      dataMembers,
      e
  ...
  public static void arrangeTokens(Editor e) {
    if (
      !e.currentFile.getName().endsWith(".java") ||
      Screen.getFileView().getProjectManager().non_java ||
      !DataManager.isContentModeJava()
    ) {
      arrangeTokens(
        e,
        CodeFramework.getCodeIgnoreDot(e.getText(), e.getCaretPosition())
      );
      return;
    }
    ...
    String text = CodeFramework.getCodeDoNotEliminateDot(
      e.getText(),
      e.getCaretPosition()
    );
    ...
    if (
      Screen.getFileView().getJDKManager() == null &&
      !Screen.getFileView().getProjectManager().non_java
    ) return;

    if (
      !text.contains(".") ||
      !CodeFramework.think(e, e.getText(), e.getCaretPosition())
    ) {
      SourceReader reader = new SourceReader(e.getText());
      LinkedList<DataMember> dataMembers = new LinkedList<>();
      ...
      if (!text.contains(".")) {
        ...
        for (SourceReader.Import im : reader.imports) {
          if (im.isStatic) {
            if (decompiledStaticDataFromClasses.contains(im.pack)) continue;

            if (CodeFramework.isSource(im.pack)) staticMembers =
              new SourceReader(CodeFramework.getContent(im.pack))
                .getDataMembers("static"); else staticMembers =
              Screen
                .getFileView()
                .getJDKManager()
                .prepareReader(im.pack)
                .getDataMembers("static");
      ...
      main:for (SourceReader.Import im : reader.imports) {
        if (
          !im.isStatic &&
          (
            im.name.contains(text) ||
            CodeFramework.isUpperCaseHintType(im.name, text)
          )
        ) {
          for (SourceReader.Import x : imports) {
            if (x.get().equals(im.get())) continue main;
          }
          imports.add(im);
        }
      }
      ...
      int k = 0;
      for (SourceReader.Import im : imports) I[k++] = im;
      imports.clear();
      for (int i = 0; i < k; i++) {
      ...
      for (SourceReader.Import im : I) imports.add(im);
      ...
      for (DataMember m : reader.dataMembers) {
        if (
          m.name.contains(text) || CodeFramework.isUpperCaseHintType(m, text)
        ) {
          dataMembers.add(m);
        }
      }
      ...
      if (!reader.dataBlocks.isEmpty()) block = reader.dataBlocks.getLast();

      if (!reader.recordingInternal) {
        if (block != null) {
          for (DataMember m : block.depthMembers) {
            if (m.name.contains(text)) {
              inner:for (DataMember mx : dataMembers) {
                if (
                  mx.name.equals(m.name) &&
                  mx.parameterCount == m.parameterCount
                ) {
