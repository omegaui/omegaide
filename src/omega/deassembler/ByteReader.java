...
public class ByteReader {
  ...
  public ByteReader(Class c) {
    if (c != null) {
      ...
      if (modifier.contains(" ")) {
        if (
          modifier.contains("interface") ||
          modifier.contains("enum") ||
          modifier.contains("record")
        ) {
  ...
  public void loadInternalClasses() {
    ...
    if (internalClasses != null && internalClasses.length != 0) {
      for (Class internalClass : internalClasses) internalReaders.add(
        new ByteReader(internalClass)
      );
    }
  }
  ...
  public void loadFields() {
    for (Field f : c.getFields()) {
      ...
      if (!member.startsWith("public")) continue;
  ...
  public void loadMethods() {
    for (Method m : c.getMethods()) {
      ...
      if (member.charAt(index + 1) != ')') parameters =
        member.substring(member.indexOf('(') + 1, member.indexOf(')'));
      ...
      dataMembers.add(
        new DataMember(access, modifier, type, name + "()", parameters)
      );
    }
  }
  ...
  public boolean isInternalReader(String className) {
    for (ByteReader r : internalReaders) {
      String name = r.className.contains("$")
        ? r.className.substring(r.className.lastIndexOf('$') + 1)
        : r.className;
  ...
  public boolean isSubClass(String className) {
    ...
    if (!name.equals("java.lang.Object")) value =
      Screen
        .getFileView()
        .getJDKManager()
        .prepareReader(name)
