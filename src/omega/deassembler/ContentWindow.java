...
public class ContentWindow extends JPanel implements KeyListener {
  ...
  private class HintComp extends JPanel {
    ...
    public String[] getHighlights() {
      if (CodeFramework.isUpperCaseHintType(match, match)) {
        ...
        int k = 0;
        while (k < S.length) S[k] = match.charAt(k++) + "";
        return S;
      }
    ...
    public Color getColorShade() {
      if (d.modifier.contains("synchronized")) return TOOLMENU_COLOR5_SHADE;
      if (d.modifier.contains("final")) return TOOLMENU_COLOR1_SHADE;
      if (d.modifier.contains("volatile")) return TOOLMENU_COLOR3_SHADE;
      return d.isMethod() ? TOOLMENU_COLOR4_SHADE : TOOLMENU_COLOR2_SHADE;
    }

    public BufferedImage getIcon() {
      if (
        d.modifier.contains("synchronized")
      ) return IconManager.fluentsyncImage;
      if (d.modifier.contains("final")) return IconManager.fluentconstantImage;
      if (
        d.modifier.contains("volatile")
      ) return IconManager.fluentvolatileImage;
      return d.isMethod()
        ? IconManager.fluentmethodImage
        : IconManager.fluentvariableImage;
    }
  }
  ...
  public void genView(LinkedList<DataMember> dataMembers, Graphics g) {
    ...
    optimalHintHeight =
      optimalHintHeight < 20 ? MINIMUM_HINT_HEIGHT : optimalHintHeight;
    ...
    final String match = CodeFramework.getCodeIgnoreDot(
      e.getText(),
      e.getCaretPosition()
    );
    ...
    dataMembers.forEach(d -> {
      if (d.getRepresentableValue() != null) {
        HintComp hintComp = new HintComp(
          d,
          match,
          () -> {
            ContentWindow.this.setVisible(false);
            String lCode = CodeFramework.getCodeIgnoreDot(
              editor.getText(),
            ...
            try {
              e
                .getDocument()
                .remove(e.getCaretPosition() - match.length(), match.length());

              e.insert(d.name, e.getCaretPosition());

              if (d.parameterCount > 0) e.setCaretPosition(
                e.getCaretPosition() - 1
              );
            } catch (Exception es) {
              es.printStackTrace();
            }
  ...
  public void decideLocation() {
    ...
    if (e.getCaret().getMagicCaretPosition() == null) return;
  ...
  public static synchronized void sort(LinkedList<DataMember> dataMembers) {
    ...
    for (Object obj : members) {
      ...
      if (m.parameters == null) vars.add(m); else meths.add(m);
    }
    dataMembers.clear();
