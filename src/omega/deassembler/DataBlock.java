      ...
      if (block.parameters.contains(",") && block.parameters.contains(" ")) {
        ...
        while (tok.hasMoreTokens()) {
          ...
          if (p.contains(" ")) {
            ...
            String type = reader.evaluateType(
              p.substring(0, p.indexOf(' ')).trim()
            );
            if (type != null) depthMembers.add(
              new DepthMember("", "", type, name, null, 1)
            );
          }
        }
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (
        ...
        if (part.contains("=")) { //Verifying the line still contains '='
          ...
          if (type != null) depthMembers.add(
            new DepthMember("", "", type, name, null, braces)
          );
        }
        continue;
      }
      ...
      if (
        ...
        if (line.contains("(") && line.contains(")")) {
          ...
          if (line.contains(";")) {
            ...
            String type = reader.evaluateType(
              exp.substring(0, exp.indexOf(' ')).trim()
            );
            if (name.contains("(") || name.contains(")")) continue;
            if (type != null) depthMembers.add(
              new DepthMember("", "", type, name, null, braces)
