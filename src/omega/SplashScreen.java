...
public class SplashScreen extends JFrame {
  ...
  private static GradientPaint gradient2 = new GradientPaint(
    100,
    150,
    isDarkMode() ? TOOLMENU_COLOR3 : TOOLMENU_COLOR1,
    300,
    300,
    TOOLMENU_COLOR4
  );
  private static BufferedImage image = (BufferedImage) omega.utils.IconManager
    .getImageIcon("/omega_ide_icon128.png")
    .getImage();
  ...
  public SplashScreen() {
    ...
    setResizable(false);
    setShape(
      new RoundRectangle2D.Float(0, 0, getWidth(), getHeight(), 100, 100)
    );

    try {
      setIconImage(
        ImageIO.read(getClass().getResourceAsStream("/omega_ide_icon500.png"))
      );
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public void render() {
    ...
    g.drawString(
      NAME,
      getWidth() / 2 - g.getFontMetrics().stringWidth(NAME) / 2,
      189
    );
    g.setFont(PX26);
    ...
    g.drawString(
      EDITION,
      getWidth() / 2 - g.getFontMetrics().stringWidth(EDITION) / 2,
      190 + 30
    );
    g.setFont(PX20);
    g.drawString(
    ...
    g.drawString(
      ENCOURAGE,
      getWidth() / 2 - g.getFontMetrics().stringWidth(ENCOURAGE) / 2,
      getHeight() - 30
    );
    g.setColor(PROGRESS_COLOR);

    if (Animations.isAnimationsOn()) {
      if (x == 30) ground = false; else if (x == 68) ground = true;
  ...
  public void setProgress(int progress, String status) {
    ...
    if (progress < 85) SplashScreen.ENCOURAGE =
      status; else SplashScreen.ENCOURAGE = "lets code";
    repaint();
  }
}
