...
public class OverView extends JDialog {
  ...
  public void init() {
    ...
    NoCaretField searchField = new NoCaretField(
      "",
      "search any method here",
      TOOLMENU_COLOR2,
      c2,
      TOOLMENU_COLOR3
    );
  ...
  public void generate() {
    ...
    comps.forEach(c -> {
      if (c.color2 == TOOLMENU_COLOR3) selections.add(
        members.get(comps.indexOf(c))
      );
    });
    ...
    new Thread(() -> {
      ...
      if (reader.features != null) {
        for (String feature : reader.features) {
          ...
          if (CodeFramework.isSource(path)) srs.add(
            new SourceReader(CodeFramework.getContent(path))
          ); else {
            ByteReader bx = null;
            if (Assembly.has(path)) bx = Assembly.getReader(path); else bx =
              omega.Screen.getFileView().getJDKManager().prepareReader(path);
            brs.add(bx);
          }
        }
      }
      ...
      for (ByteReader b : brs) {
        ...
        for (DataMember d : b.dataMembers) {
          if (
            d.parameters != null &&
            !Generator.isMemberOfObject(d) &&
            d.modifier != null &&
            !d.modifier.contains("final")
          ) {
