...
public class Generator {
  ...
  public static String getSuitableName(String name) {
    if (Character.isLowerCase(name.charAt(0))) return name.charAt(0) + "";
    if (name.contains("<")) return (
      name.substring(0, 1).toLowerCase() + name.substring(1, name.indexOf("<"))
    );
    if (name.contains("[")) return (
  ...
  public static boolean isMemberOfObject(DataMember d) {
    if (reader == null) {
      if (Assembly.has("java.lang.Object")) reader =
        Assembly.getReader("java.lang.Object"); else reader =
        omega.Screen
          .getFileView()
          .getJDKManager()
          .prepareReader("java.lang.Object");
    }
    for (DataMember dx : reader.dataMembers) {
      if (dx.toString().equals(d.toString())) return true;
    }
    return false;
  }
  ...
  public static void implement(DataMember d, RSyntaxTextArea textArea) {
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (token.contains(".")) token =
        token.substring(token.lastIndexOf('.') + 1).trim();
      String data = getSuitableName(token);
      try {
        if (
          Character.isLetter(
            parameters
              .substring(parameters.indexOf(token) + token.length())
              .trim()
              .charAt(0)
          )
        ) data = tok.nextToken();
    ...
    if (rep.contains(",")) rep = rep.substring(0, rep.lastIndexOf(','));
    rep += ")";
    String type = d.type;
  ...
  public static void genGetter(
    ...
    String tabs = getTabs(textArea, caret);
    String meth =
      access + (d.modifier.contains("volatile") ? "" : (" " + d.modifier));
    meth = meth.trim() + " " + d.type;
    String name =
      (d.type.equals("boolean") ? "is" : "get") + toUpperCase(d.name) + "() {";
    meth = meth.trim() + " " + name;
    meth +=
  ...
  public static void genSetter(
    ...
    meth = meth.trim() + " void";
    String name =
      "set" + toUpperCase(d.name) + "(" + d.type + " " + d.name + ") {";
    meth = meth.trim() + " " + name;
