...
public class ErrorHighlighter {
  ...
  public static final Color color = omega.utils.UIManager.isDarkMode()
    ? Color.decode("#800707")
    ...
    try {
      while (tokenizer.hasMoreTokens()) {
        ...
        if (token.contains(".java") && token.contains(":") && !canRecord) {
          int e = token.indexOf(':');
          if (Screen.onWindows()) e = token.indexOf(':', e + 1);
          path = token.substring(0, e);
          path = JavaSyntaxParser.convertToProjectPath(path);
          line =
            Integer.parseInt(token.substring(e + 1, token.indexOf(':', e + 1)));
          if (new File(path).exists()) canRecord = true;
        } else if (canRecord) {
          code = token.trim();
          ...
          HighlightPainter hp = new DefaultHighlighter.DefaultHighlightPainter(
            color
          );
