...
public class DataBase {
  ...
  public DataBase(String baseName) {
    ...
    if (dataBaseFile.exists()) readDataBase();
  }
  ...
  public synchronized void save() {
    try {
      ...
      for (String name : names) {
        ...
        for (DataEntry e : en) writer.println(VALUE_BORDER + e.getValue());
      }
      writer.close();
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public LinkedList<DataEntry> getEntries(String set) {
    ...
    for (DataEntry e : entries) {
      if (e.getName().equals(set)) entriesX.add(e);
    }
    return entriesX;
  }
