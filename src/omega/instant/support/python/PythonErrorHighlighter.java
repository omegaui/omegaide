  ...
  public void loadErrors(String errorLog) {
    ...
    try {
      while (tokenizer.hasMoreTokens()) {
        ...
        if (
          token.trim().startsWith("Traceback (most recent call last):")
        ) continue;
        if (!canRecord && token.contains(", line")) {
          ...
          path =
            token
              ...
              .trim();
          String lineString = token
            .substring(token.indexOf(", line") + ", line".length() + 1)
            .trim();
          if (lineString.contains(",")) lineString =
          ...
          if (token.equals("^")) message = token = tokenizer.nextToken().trim();
    ...
    for (Highlight h : highlights) {
      ...
      for (Highlight hx : highlights) {
        if (h.editor.equals(hx.editor)) count++;
      }
