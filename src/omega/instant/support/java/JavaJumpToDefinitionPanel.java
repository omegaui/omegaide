...
public class JavaJumpToDefinitionPanel extends JPanel {
  ...
  public void genDefinitions(String match) {
    try {
      ...
      for (DataMember dx : reader.ownedDataMembers) {
        if (!dx.getRepresentableValue().contains(match)) continue;
        int width = g.getFontMetrics().stringWidth(dx.name + " - " + dx.type);
        if (width > min_width) min_width = width + 15;
      }
          ...
          TextComp comp = new TextComp(
            ...
            () -> {
              editor.setCaretPosition(
                JavaCodeNavigator.getLineOffset(editor, dx.lineNumber)
              );
              setVisible(false);
            }
          );
            ...
            () -> {
              ...
              try {
                editor
                  ...
                  .setValue(
                    JavaCodeNavigator.getLineOffsetForView(
                      editor,
                      dx.lineNumber
                    )
                  );
      ...
      if (getWidth() > 400) setSize(400 + 20, getHeight());
      if (getHeight() > 350) setSize(getWidth(), 350);
  ...
  public void relocate() {
    ...
    setLocation(
      rect.x + rect.width / 2 - getWidth() / 2,
      rect.y + rect.height / 2 - getHeight() / 2
    );
    layout();
  }
