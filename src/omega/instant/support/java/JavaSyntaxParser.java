...
public class JavaSyntaxParser {
  ...
  public static final File BUILDSPACE_DIR = new File(
    ".omega-ide",
    "buildspace"
  );

  public static NotificationPopup dynamicBuildInfoPopup = NotificationPopup
  ...
  public synchronized void parse() {
    if (parsing || packingCodes) return;
    new Thread(() -> {
      errorCount = 0;
      warningCount = 0;
      ...
      if (diagnostics == null || diagnostics.getDiagnostics() == null) return;

      List<Diagnostic<? extends JavaFileObject>> diagnosticList = diagnostics.getDiagnostics();
      for (Diagnostic d : diagnosticList) {
        if (
          d.getKind() != Kind.ERROR &&
          d.getKind() != Kind.WARNING &&
          d.getKind() != Kind.MANDATORY_WARNING
        ) continue;
        ...
        filePath =
          ...
          filePath.substring(srcDir.length() + (Screen.onWindows() ? 1 : 0));
      ...
      main:for (Highlight h : highlights) {
        for (JavaErrorData data : datas) {
          if (data.editor == h.editor) continue main;
        }
      ...
      if (totalErrors > 0 || totalWarnings > 0) omega.Screen
        .getScreen()
        .getToolMenu()
        .setTask(
        ...
        .getScreen()
        .getToolMenu()
        .setTask("Currently Opened Codes are Error Free!");
  ...
  public DiagnosticCollector<JavaFileObject> compile() {
    ...
    try {
      ...
      options.add("-d");
      options.add(
        ".omega-ide" + File.separator + "buildspace" + File.separator + "bin"
      );
  ...
  public DiagnosticCollector<JavaFileObject> compileAndSaveToProjectBin() {
    ...
    try {
      ...
      for (String filePath : files) {
        ...
        bytePath =
          bytePath.substring(BUILDSPACE_DIR.getAbsolutePath().length() + 4);
        bytePath =
          Screen.getFileView().getProjectPath() +
          File.separator +
          "bin" +
        ...
        for (File fx : F) {
          if (!fx.getName().endsWith(".class")) continue;
          if (
            fx.getName().equals(name) ||
            (
              fx.getName().startsWith(simpleName) &&
              fx.getName().charAt(simpleName.length()) == '$'
            )
          ) {
            fx.delete();
          }
  ...
  public DiagnosticCollector<JavaFileObject> compileFullProject() {
    ...
    try {
      ...
      Scanner reader = new Scanner(
        new File(
          Screen.getFileView().getProjectPath() + File.separator + ".sources"
        )
      );
  ...
  public static void prepareBuildSpace(
    LinkedList<String> compilationUnitFiles
  ) {
    try {
      //Cleaning BuildSpace
      ...
      for (Editor editor : editors) {
        ...
        if (
          ...
          String metaPath = filePath.substring(
            filePath.indexOf(projectDir) + projectDir.length()
          );
  ...
  public static LinkedList<String> getArgs() {
    ...
    if (files != null && files.length != 0) {
      depenPath +=
        ...
        File.separator +
        "bin" +
        omega.Screen.PATH_SEPARATOR;
    } else {
      dynamicBuildInfoPopup.showIt();
      Screen.setStatus(
        "Your Must Have Build the Whole Project at least Once for carrying out correct JavaSyntaxParsing and Instant Run",
        0,
        IconManager.fluentbriefImage
      );
    }

    if (Screen.isNotNull(depenPath)) depenPath =
  ...
  public static void loadFiles(LinkedList<String> files, File dir, String ext) {
    ...
    for (File file : F) {
      if (file.isDirectory()) loadFiles(files, file, ext); else if (
  ...
  public static Color getSuitableColor(Kind kind) {
    if (
      kind == Kind.ERROR || kind == Kind.MANDATORY_WARNING
  ...
  public static String convertToProjectPath(String buildSpacePath) {
    if (
      !buildSpacePath.startsWith(BUILDSPACE_DIR.getAbsolutePath())
    ) return buildSpacePath;
    String path =
      Screen.getFileView().getProjectPath() +
      (Screen.onWindows() ? File.separator : "");
    buildSpacePath =
      buildSpacePath.substring(
        BUILDSPACE_DIR.getAbsolutePath().length() + (Screen.onWindows() ? 1 : 0)
      );
