...
import omega.instant.support.java.assist.JarLoader;
...
public class JDKManager {
	...
	public void readRTJarFile() throws Exception{
		...
		try{
			...
			try(JarFile rtJarFile = new JarFile(rtJarPath)){
				for(Enumeration<JarEntry> enums = rtJarFile.entries(); enums.hasMoreElements();) {
					...
					if(!name.endsWith("/")){
						...
						if(classPath != null){
							if(systemJarLoader.isAccessible(classPath)){
			...
			e.printStackTrace();
			throw new Exception("Exception while Reading JDK v" + version);
		}
	}
	...
	public void readModules(){
		...
		modules.forEach((module)->{
			module.classes.forEach(c->{
				if(systemJarLoader.isAccessible(c.getImport())){
					addImport(c);
