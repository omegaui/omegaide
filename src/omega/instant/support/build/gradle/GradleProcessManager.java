    ...
    new Thread(() -> {
      try {
        ...
        if (!Screen.getFileView().getProjectManager().non_java) {
          ...
          Screen
            .getScreen()
            .manageTools(Screen.getFileView().getProjectManager());
  ...
  public static void run() {
    new Thread(() -> {
      try {
        ...
        JetRunPanel printArea = new JetRunPanel(
          false,
          new String[] { DataManager.getGradleCommand() + ext, "run" },
          Screen.getFileView().getProjectPath()
        );
        printArea.launchAsTerminal(
          GradleProcessManager::run,
          IconManager.fluentgradleImage,
          DataManager.getGradleCommand() + " run"
        );

        printArea.print(
          "# Executing : " + DataManager.getGradleCommand() + " run"
        );
        printArea.print("--------------------------------------------------");
  ...
  public static void build() {
    new Thread(() -> {
      try {
        ...
        JetRunPanel printArea = new JetRunPanel(
          false,
          new String[] { DataManager.getGradleCommand() + ext, "build" },
          Screen.getFileView().getProjectPath()
        );
        printArea.setLogMode(true);
        printArea.print(
          "# Executing : " + DataManager.getGradleCommand() + " build"
        );
        printArea.print("--------------------------------------------------");
