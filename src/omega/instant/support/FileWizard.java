...
public class FileWizard extends JDialog {
  ...
  private void init() {
    iconComp =
      new TextComp(IconManager.fluentnewItemImage, 48, 48, back2, c2, c2, null);
    ...
    putAnimationLayer(
      iconComp,
      getImageSizeAnimationLayer(25, -6, true),
    ...
    final OPopupWindow popup = new OPopupWindow(
      "File-Type Menu",
      this,
      0,
      false
    ...
    popup
      ...
      .createItem(
        ...
        () -> typeBtn.setText("enum")
      )
      .createItem(
        "Custom File",
        IconManager.fileImage,
        () -> typeBtn.setText("Custom File")
      );
    typeBtn.addMouseListener(
      new MouseAdapter() {
        @Override
  ...
  public void create() {
    ...
    if (type.equals("directory")) {
      File dir = new File(
        parentRoot.getToolTipText() + File.separator + nameField.getText()
      );
      dir.mkdir();
    ...
    if (!type.equals("Custom File")) {
      ...
      File src = new File(
        path + text.substring(text.lastIndexOf('.') + 1) + ".java"
      );
      ...
      } else nameField.setText("File Already Exists");
    }
  }
  ...
  public void show(String type) {
    ...
    if (
      Screen.getFileView().getProjectPath() != null &&
      new File(Screen.getFileView().getProjectPath()).exists()
    ) parentRoot.setToolTipText(
      Screen.getFileView().getProjectPath() + File.separator + "src"
    );
    setVisible(true);
  }

  public static void createSRCFile(
    File file,
    String type,
    String pack,
    String name
