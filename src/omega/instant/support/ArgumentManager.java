...
public class ArgumentManager extends DataBase {
   ...
   */
  public void load() {
    if (getEntries("Compile Time Argument") != null) getEntries(
      "Compile Time Argument"
    )
      .forEach(entry -> compile_time_args.add(entry.getValue()));
    if (getEntries("Run Time Argument") != null) getEntries("Run Time Argument")
      .forEach(entry -> run_time_args.add(entry.getValue()));
    compileDir =
      getEntryAt("Compile Time Working Directory", 0) != null
        ? getEntryAt("Compile Time Working Directory", 0).getValue()
        : "";
    runDir =
      getEntryAt("Run Time Working Directory", 0) != null
        ? getEntryAt("Run Time Working Directory", 0).getValue()
        : "";

    if (!new File(compileDir).exists()) compileDir = "";
    if (!new File(runDir).exists()) runDir = "";
  ...
  public void genLists() {
    units.forEach(unit -> {
      ...
      loadFiles(
        unit.getFileExtension(),
        files,
        new File(unit.getWorkingDirectory())
      );
      if (!files.isEmpty()) writeList(
        unit.getContainerName(),
        files,
        unit.isQuoted()
      );
    });
  ...
  public void writeList(String name, LinkedList<File> files, boolean sur) {
    try {
      PrintWriter writer = new PrintWriter(
        new File(Screen.getFileView().getProjectPath() + File.separator + name)
      );
      files.forEach(file -> {
        if (sur) writer.println(
          "\"" + file.getAbsolutePath() + "\""
        ); else writer.println(file.getAbsolutePath());
      });
      writer.close();
    } catch (Exception e) {
  ...
  public void loadFiles(String ext, LinkedList<File> files, File dir) {
    ...
    for (File fx : F) {
      if (!fx.isDirectory() && fx.getName().endsWith(ext)) files.add(
        fx
      ); else if (fx.isDirectory()) loadFiles(ext, files, fx);
    }
  }
