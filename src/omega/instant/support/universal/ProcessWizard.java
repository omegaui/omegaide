...
public class ProcessWizard extends JDialog {
  ...
  public void init() {
    ...
    cmdField =
      new TextComp(
        ...
        TOOLMENU_COLOR2,
        () -> {
          if (!Screen.isNotNull(extField.getText())) return;
          commandWindow.loadView(
            processManager.getExecutionCommand(new File(extField.getText()))
          );
    ...
    scrollPane.setBounds(
      10,
      10,
      containerPanel.getWidth() - 20,
      containerPanel.getHeight() - 20
    );
  ...
  public void genView() {
    ...
    ProcessManager.dataSet.forEach(data -> {
      TextComp comp = new TextComp(
        data.fileExt,
        data.executionCommand.toString(),
  ...
  public void removeData(ProcessData data) {
    ...
    if (items.isEmpty()) setSize(400, 200); else setSize(400, 500);
    setLocationRelativeTo(null);
  }
  ...
  public void addCurrent() {
    ...
    int pointer =
      scrollPane.getVerticalScrollBar().getValue() + (done ? 0 : 25);
    genView();
  ...
  public void launch(File file) {
    if (file != null && file.exists() && file.getName().contains(".")) {
      ...
      int res = ChoiceDialog.makeChoice(
        "No Execution Command available for " + ext + " files!",
        "Add One",
        "Quit"
      );
