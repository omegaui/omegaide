...
public class ToolMenuPathBox extends JComponent {
  ...
  public void prepareEdges() {
    ...
    if (path.startsWith(Screen.getFileView().getProjectPath())) path =
      path.substring(Screen.getFileView().getProjectPath().length());
    block = 0;
    LinkedList<String> tokens = CodeTokenizer.tokenize(
      path,
      File.separatorChar
    );
