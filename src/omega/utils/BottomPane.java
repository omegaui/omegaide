...
public class BottomPane extends JPanel {
  ...
  public void init() {
    messageComp =
      new TextComp(
        "Status of any process running will appear here!",
    ...
    messageComp.addMouseListener(
      new MouseAdapter() {
        @Override
        public void mousePressed(MouseEvent e) {
          if (e.getButton() == 1 && e.getClickCount() == 2) r.run();
        }
      }
    );
    add(messageComp);
    ...
    logComp =
      new TextComp("No Logs", TOOLMENU_COLOR4_SHADE, back1, glow, null) {
        @Override
        public void draw(Graphics2D g) {
          if (image != null) {
            g.drawImage(
    ...
    jumpField.addActionListener(e -> {
      if (!jumpField.hasText()) return;
      String text = jumpField.getText();
      for (char c : text.toCharArray()) {
        if (!Character.isDigit(c)) return;
      }
      int line = Integer.parseInt(text);
      String code = Screen.getScreen().getCurrentEditor() != null
        ? Screen.getScreen().getCurrentEditor().getText()
        : "";
    ...
    putAnimationLayer(
      searchComp,
      getImageSizeAnimationLayer(25, 5, false),
  ...
  public void toggleFocusMode() {
    ...
    focusComp.image =
      screen.isFocusMode()
        ? IconManager.fluentnormalScreenImage
        : IconManager.fluentfocusImage;
    focusComp.repaint();
  }
