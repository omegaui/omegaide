...
public class BuildLog extends JPanel {
  ...
  public BuildLog() {
    ...
    headComp =
      new TextComp(
        "Build Resulted in the following Error(s)",
    ...
    splitPane.setLeftComponent(
      fileScrollPane = new JScrollPane(filePanel = new JPanel(null))
    );
    splitPane.setRightComponent(
      errorScrollPane = new JScrollPane(errorArea = new RTextArea())
    );
  ...
  public void genView(String log) {
    ...
    while (tok.hasMoreTokens()) {
      ...
      if (line.contains(".java:")) {
        if (!justStarted) errors.add(
          new Error(
            JavaSyntaxParser.convertToProjectPath(filePath),
            lineN,
            error
          )
        );
        ...
        if (File.separator.equals("\\")) filePath =
          ...
          ); else filePath = line.substring(0, line.indexOf(':'));
        if (!filePath.equals("")) {
          if (File.separator.equals("\\")) {
            ...
            error =
              line.substring(line.indexOf(':', line.indexOf(':') + 1) + 1);
          }
          error = error.trim() + "\n" + "\n";
        }
      } else {
    ...
    if (!filePath.equals("")) errors.add(
      new Error(JavaSyntaxParser.convertToProjectPath(filePath), lineN, error)
    );
    ...
    errors.forEach(errorSet -> {
      int w =
        g
          .getFontMetrics()
          .stringWidth(errorSet.getFileName() + " : " + errorSet.line) +
        20;
      if (w > maxW) maxW = w;
    });
    errors.forEach(errorSet -> {
      TextComp errorComp = new TextComp(
        errorSet.getFileName() + " : " + errorSet.line,
      ...
      block += 25;
      TextComp fixedComp = new TextComp(
        "fixed",
  ...
  public void setView(Error errorSet) {
    ...
    if (editorX != null) editorX.grabFocus();
    ...
    try {
      ...
      Color color = isDarkMode()
        ? omega.utils.UIManager.TOOLMENU_COLOR2_SHADE
        : ErrorHighlighter.color;
      ...
      new Thread(() -> {
        Editor editor = Screen
          .getScreen()
          .getTabPanel()
          .findEditor(new File(errorSet.filePath));
        if (editor != null) {
          ...
          for (int i = 0; i < textM.length(); i++) {
            ...
            if (lineN == errorSet.line) {
              Screen
                ...
                .getTabPane()
                .setSelectedIndex(
                  Screen.getScreen().getTabPanel().getEditors().indexOf(editor)
                );
  ...
  public void paint(Graphics graphics) {
    if (fileComps.isEmpty()) {
      ...
      g.drawString(
        headComp.getText(),
        getWidth() / 2 - g.getFontMetrics().stringWidth(headComp.getText()) / 2,
        getHeight() /
        2 -
