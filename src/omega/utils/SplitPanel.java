  ...
  public void analyzeDivider() {
    if (splitType == HORIZONTAL_SPLIT) {
      if (
        ...
        getLeftComponent().getWidth() == 0 ||
        getRightComponent().getWidth() == 0 ||
        !getLeftComponent().isVisible() ||
        !getRightComponent().isVisible()
      ) {
        dividerSize = getDividerSize() > 0 ? getDividerSize() : dividerSize;
        setDividerSize(0);
      } else setDividerSize(dividerSize);
    } else {
      if (
        getTopComponent() == null ||
        getBottomComponent() == null ||
        getBottomComponent().getWidth() == 0 ||
        getTopComponent().getWidth() == 0 ||
        !getTopComponent().isVisible() ||
        !getBottomComponent().isVisible()
      ) {
        dividerSize = getDividerSize() > 0 ? getDividerSize() : dividerSize;
        setDividerSize(0);
      } else setDividerSize(dividerSize);
