...
public class TabComp {
  ...
  public static FlexPanel create(
    Component c,
    ...
    TextComp closeButton = new TextComp(
      "x",
    ...
    TextComp comp = new TextComp(
      ...
      TOOLMENU_GRADIENT,
      c2,
      FileTreeBranch.getPreferredColorForFile(new File(name)),
      null
    );
    comp.setArc(6, 6);
    ...
    final MouseAdapter mouseAdapter = new MouseAdapter() {
      ...
      public void mousePressed(MouseEvent e) {
        areas.forEach(a -> {
          if (a != comp) a.setColors(
            TOOLMENU_GRADIENT,
            c2,
            FORE
          ); else a.setColors(TOOLMENU_GRADIENT, c2, glow);
          a.repaint();
        });
        focusAction.run();
      }
    ...
    if (image == null) {
      if (c instanceof Editor) {
        image =
          FileTreeBranch.getPreferredImageForFile(((Editor) c).currentFile);
      } else image = IconManager.fluentshellImage;
    }
    if (popUp == null) alpha = c2;

    iconButton =
      new TextComp(image, 25, 25, toolTip, alpha, c2, FORE, () -> {});
    iconButton.setArc(0, 0);
    iconButton.setPreferredSize(
      new Dimension(
        baseName.length() > 2 ? (baseName.length() > 3 ? 40 : 30) : 28,
        28
      )
    );
    ...
    putAnimationLayer(
      iconButton,
      getImageSizeAnimationLayer(20, -4, true),
    ...
    areas.forEach(a -> {
      if (a != comp) a.setColors(TOOLMENU_GRADIENT, c2, FORE); else a.setColors(
        TOOLMENU_GRADIENT,
        c2,
        glow
      );
  ...
  public static String getBaseName(String ext) {
    if (ext.equals("Compilation")) return "JVM";

    if (ext.equals("Building")) return "IDE"; else if (
      ext.equals("Terminal")
    ) return "Shell"; else if (ext.equals("File Operation")) return "Task";

    if (ext.contains("Run(")) return "JVM";

    if (
      (ext.contains("Run ") || ext.contains("Run") || ext.contains("Build"))
    ) return "IDE";

    if (!ext.contains(".")) return "?";

    if (
      ext.equals(".projectInfo") ||
      ext.equals(".sources") ||
      ext.equals(".args") ||
      ext.equals(".preferences") ||
      ext.equals(".recents") ||
      ext.equals(".firststartup") ||
      ext.equals(".ui") ||
      ext.equals(".plugs") ||
      ext.equals(".snippets")
    ) return "IDE";
