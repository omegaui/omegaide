...
public class ExtendedBuildPathManager extends JDialog {
  ...
  public void init() {
    titleComp =
      new TextComp("Add Additional Flags", TOOLMENU_COLOR3, c2, c2, null);
  ...
  public void setVisible(boolean value) {
    if (value) {
      compilePanel.loadFlags(
        Screen.getFileView().getProjectManager().compileTimeFlags
      );
  ...
  public void dispose() {
    ...
    try {
      ...
      getCompileTimeFlags()
        .forEach(
          Screen.getFileView().getProjectManager().compileTimeFlags::add
        );
      getRunTimeFlags()
        .forEach(Screen.getFileView().getProjectManager().runTimeFlags::add);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
