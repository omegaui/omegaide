...
public class FileView extends View {
  ...
  public void readJDK() {
    if (projectManager.jdkPath == null) return;

    int version = 0;
    if (jdkManager != null) version = jdkManager.getVersionAsInt();

    int versionThis = JDKManager.calculateVersion(
      new File(projectManager.jdkPath)
    );
    if (version != versionThis) Assembly.deassemble();

    if (jdkManager != null) jdkManager.clear();
  ...
  public void setProjectPath(String path) {
    ...
    if (projectPath != null && projectPath.equals(path)) return;
    ...
    if (Screen.launcher != null) Screen.launcher.dispose();
    ...
    if (projectManager.non_java) argumentManager = new ArgumentManager();

    if (!projectManager.non_java) {
      if (
        Screen.getFileView().getProjectManager().jdkPath == null ||
        !new File(Screen.getFileView().getProjectManager().jdkPath).exists()
      ) Screen.notify(
        "No JDK Defined for Project " + Screen.getFileView().getProjectName(),
        3000,
        null
      ); else readJDK();
    ...
    Screen
      ...
      .triggerReaction(
        PluginReactionEvent.genNewInstance(
          PluginReactionEvent.EVENT_TYPE_PROJECT_CHANGED,
          this,
          projectPath
        )
      );
  ...
  public boolean open(String type) {
    ...
    if (type.equals("Project")) {
      ...
      if (!files.isEmpty()) {
        for (File file : files) getScreen().loadFile(file);
      }
    }
    return false;
  }
  ...
  public void closeProject() {
    ...
    if (Screen.launcher == null) Screen.launcher =
      new omega.launcher.Launcher();
    Screen
      .getPluginReactionManager()
      .triggerReaction(
        PluginReactionEvent.genNewInstance(
          PluginReactionEvent.EVENT_TYPE_PROJECT_CLOSED,
          this,
          projectPath
        )
      );
  ...
  public String getProjectName() {
    ...
    return projectPath.substring(
      projectPath.lastIndexOf(File.separatorChar) + 1
    );
  }
