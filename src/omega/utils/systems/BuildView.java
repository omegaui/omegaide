...
public class BuildView extends View {
  ...
  public static LinkedList<File> loadAllFiles(
    String dir,
    LinkedList<File> files
  ) {
    File[] fs = new File(dir).listFiles();
    for (File f : fs) {
      if (f.isDirectory()) loadAllFiles(
        f.getAbsolutePath(),
        files
      ); else files.add(f);
    }
  ...
  public static String[] convertToArray(String args) {
    ...
    for (int i = 0; i < args.length(); i++) {
      ...
      if (ch == '\"' && !strRec) strRec = true; else if (
        ch == '\"' && strRec
      ) strRec = false;
      if (ch == ' ' && !strRec) {
        arguments.add(token);
    ...
    int k = 0;
    for (String x : arguments) A[k++] = x;
    return A;
  }

  public void compileProject() {
    if (compileProcess != null) {
      if (compileProcess.isAlive()) return;
    }
    ...
    new Thread(() -> {
      try {
        ...
        if (Screen.isNotNull(jdkPath) && new File(jdkPath).exists()) cmd =
  ...
  public static void loadClassFiles(File out, LinkedList<File> files) {
    ...
    for (File f : F) {
      if (f.isDirectory()) loadClassFiles(f, files); else if (
  ...
  public void deleteDir(File file) {
    if (file.isDirectory()) {
      if (file.list().length == 0) deleteEmptyDir(file); else {
  ...
  public void createClassList() {
    ...
    try {
      ...
      loadData(
        dirs,
        new File(Screen.getFileView().getProjectPath() + File.separator + "src")
          .listFiles()
      );
      ...
      classess.forEach(c -> {
        ...
        if (Screen.onWindows()) res = "\"" + res + "\""; else res =
          "\"" + File.separator + res + "\"";
        paths.add(res);
      });
      //Creating SourcePath
      PrintWriter writer = new PrintWriter(
        new FileOutputStream(
          Screen.getFileView().getProjectPath() + File.separator + SRC_LIST
        )
      );
  ...
  private static void loadData(
    LinkedList<String> paths0,
    LinkedList<String> paths1
  ) {
    for (String path0 : paths0) paths1.add(path0);
  }
}
