...
public class RunView extends View {
  ...
  public void setMainClassPath(String mainClassPath) {
    if (!mainClassPath.endsWith(".java")) return;
    mainClass = "";
    ...
    while (tokenizer.hasMoreTokens()) {
      ...
      if (canRecord) mainClass += token + "."; else if (
        token.equals("src")
      ) canRecord = true;
    }
  ...
  public void setMainClass() {
    if (getScreen().getCurrentEditor().currentFile == null) return;
    mainClassPath =
      getScreen().getCurrentEditor().currentFile.getAbsolutePath();
    if (!mainClassPath.endsWith(".java")) return;
    mainClass = "";
    boolean canRecord = false;
    StringTokenizer tokenizer = new StringTokenizer(
    ...
    new Thread(() -> {
      ...
      if (
        ...
        try {
          ...
          Screen.setStatus(
            "Building Project -- Double Click to kill this process",
            70,
          ...
          Screen
            ...
            .setDoubleClickAction(() -> {
              ...
              if (
                printArea.terminalPanel.process.isAlive()
              ...
              getScreen()
                ...
                .setShowLogAction(() -> {
                  getScreen()
                    ...
                    .addTab(
                      "Build",
                      IconManager.fluenttesttubeImage,
                      printArea,
    ...
    new Thread(() -> {
      ...
      if (
        ...
        NotificationPopup
          ...
          .shortMessage(
            "Click \"Settings\", then Click \"All Settings\"",
            TOOLMENU_COLOR1
          )
      ...
      try {
        ...
        new Thread(() -> {
          while (
            terminal.terminalPanel.process != null &&
            terminal.terminalPanel.process.isAlive()
          );
          terminal.printText(
            "---<>--------------------------------------<>---"
          );
          if (terminal.terminalPanel.process != null) terminal.printText(
            "Program Execution finished with Exit Code " +
            terminal.terminalPanel.process.exitValue()
          ); else terminal.printText("Unable to Run the Specified Command!");
    ...
    new Thread(() -> {
      ...
      if (!Screen.isNotNull(mainClass)) {
        NotificationPopup
          ...
          .title("Project Management")
          .message(
            "No Main-Class Specified, Cancelling Run Operation",
            TOOLMENU_COLOR2
          )
          .shortMessage(
            "Click Editor's Tab Icon to Mark a class as Main-Class",
            TOOLMENU_COLOR1
          )
          .dialogIcon(IconManager.fluentfolderImage)
          .build()
      ...
      try {
        ...
        } else NATIVE_PATH = "$PATH";

        String depenPath = "";
        for (String d : Screen
          .getFileView()
          .getProjectManager()
          .jars) depenPath += d + omega.Screen.PATH_SEPARATOR;
        for (String d : Screen
          .getFileView()
          .getProjectManager()
          .resourceRoots) depenPath += d + omega.Screen.PATH_SEPARATOR;
        if (!depenPath.equals("")) {
        ...
        if (Screen.getFileView().getProjectManager().jdkPath == null) {
          getScreen().getToolMenu().runComp.setClickable(true);
          Screen.setStatus(
            "Please Setup the Project JDK First!",
            99,
            IconManager.fluentbrokenbotImage
          );
          return;
        }

        String jdkPath = String.copyValueOf(
          Screen.getFileView().getProjectManager().jdkPath.toCharArray()
        );
        String cmd = null;
        if (jdkPath != null && new File(jdkPath).exists()) cmd =
          jdkPath + File.separator + "bin" + File.separator + "java"; else cmd =
          "java" + cmd;

        File workingDir = new File(
          Screen.getFileView().getProjectPath() + File.separator + "bin"
        );
        ...
        if (Screen.isNotNull(NATIVE_PATH)) commands.add(
        ...
        Screen
          .getFileView()
          .getProjectManager()
          .runTimeFlags.forEach(commands::add);
        ...
        int k = -1;
        for (String command : commands) commandsAsArray[++k] = command;

        JetRunPanel terminal = new JetRunPanel(
          true,
          commandsAsArray,
  ...
  public void instantBuild() {
    ...
    new Thread(() -> {
      try {
        if (
          !JDKManager.isJDKPathValid(
            Screen.getFileView().getProjectManager().jdkPath
          )
        ) {
          Screen.setStatus(
            "Please first select a valid JDK for the project",
            10,
            IconManager.fluentrocketbuildImage
          );
          return;
        }
        ...
        if (!fx.exists()) {
          ...
          NotificationPopup
            ...
            .message(
              "Extremely Fast but Accuracy Mode rapidly increases Memory Footprint"
            )
            .shortMessage(
              "Use it only when creating artifacts",
              TOOLMENU_COLOR1
            )
            .iconButton(
              IconManager.fluentinfoImage,
              () -> omega.utils.ToolMenu.instructionWindow.setVisible(true),
              "See Instructions For More Detail on Tools"
            )
            .build()
            .locateOnBottomLeft()
            .showIt();
        ...
        Screen.setStatus(
          "Building Project -- Instant Build",
          0,
          IconManager.fluentrocketbuildImage
        );
        ...
        Screen.setStatus(
          "Building Project, Accomplished Successfully -- Instant Build",
          0,
          IconManager.fluentrocketbuildImage
        );
      } catch (Exception e) {
  ...
  public void instantRun() {
    ...
    new Thread(() -> {
      try {
        ...
        String text = DataManager
            .getInstantMode()
            .equals(DataManager.INSTANT_MODE_SPEED)
          ? "instant-mode-speed"
          : "instant-mode-accuracy";
        Screen.setStatus(
          "Building Project -- Instant Run : " + text,
          0,
          IconManager.fluentrocketImage
        );
        DiagnosticCollector<JavaFileObject> diagnostics = DataManager
            .getInstantMode()
            .equals(DataManager.INSTANT_MODE_SPEED)
          ? SyntaxParsers.javaSyntaxParser.compileAndSaveToProjectBin()
          : SyntaxParsers.javaSyntaxParser.compileFullProject();
        ...
        if (diagnostics == null) {
          if (
            ...
            NotificationPopup
              ...
              .message(
                "Instant Mode Speed Requires Pre-Compiled Byte Codes",
                TOOLMENU_COLOR2
              )
            ...
            NotificationPopup
              ...
              .message(
                "No Editor Present in the current session",
                TOOLMENU_COLOR4
              )
        ...
        if (!passed) {
          ...
          Screen.setStatus(
            "Avoid closing editors after editing else instant run will not be able to run successfully.",
            10,
            IconManager.fluentbrokenbotImage
          );
          System.gc();
        ...
        if (
          ...
          Screen.setStatus(
            "None Compiled Codes Present, Aborting Instant Run. Rebuild the Project First -- Instant Run",
            0,
            IconManager.fluentbrokenbotImage
          );
          System.gc();
    ...
    new Thread(() -> {
      ...
      try {
        try {
          ...
          if (Screen.getBuildView().classess.isEmpty()) return;
          ...
          Screen
            ...
            .setDoubleClickAction(() -> {
              new Thread(() -> {
                ...
                if (
                  compileProcess != null && compileProcess.isAlive()
          ...
          if (jdkPath != null && new File(jdkPath).exists()) jdkPath =
            String.copyValueOf(jdkPath.toCharArray()) +
            File.separator +
            "bin" +
            File.separator;
          ...
          if (
            !Screen.getFileView().getProjectManager().resourceRoots.isEmpty()
          ) {
            for (String d : Screen
              .getFileView()
              .getProjectManager()
          ...
          if (jdkPath != null && new File(jdkPath).exists()) cmd =
            jdkPath + "javac"; else cmd = "javac" + cmd;
          ...
          Screen
            .getFileView()
            .getProjectManager()
            .compileTimeFlags.forEach(commands::add);
          ...
          compileProcess =
            new ProcessBuilder(commandsAsArray).directory(workingDir).start();
        ...
        for (String d : Screen
          ...
          .getProjectManager()
          .resourceRoots) depenPath += d + omega.Screen.PATH_SEPARATOR;
        if (!depenPath.equals("")) depenPath =
          depenPath.substring(0, depenPath.length() - 1);
