...
public class SourceDefender extends JDialog {
  ...
  public static final String BACKUP_DIR =
    ".omega-ide" + File.separator + "backups";
  ...
  public void init() {
    ...
    titleComp =
      new TextComp(
        "Source Defender",
        "All Backups are located at " +
        (new File(BACKUP_DIR).getAbsolutePath()),
        c2,
        c2,
        glow,
        null
    ...
    TextComp openBackups = new TextComp(
      "Browse",
      "Click to open backup folder in your system's shell",
    ...
    label0.setText(
      "Automatic Backup - " +
      (DataManager.isSourceDefenderEnabled() ? "On" : "Off")
    );
    ...
    TextComp createBackupComp = new TextComp(
      "Create Backup",
      TOOLMENU_COLOR1_SHADE,
      back2,
      TOOLMENU_COLOR1,
  ...
  public void backupData() {
    ...
    final String backupTitle = new File(DataManager.getWorkspace()).getName() +
    File.separator +
    Screen.getFileView().getProjectName() +
    File.separator +
    backupTime;
  ...
  public void restoreBackup(String backupTitle) {
    int res = ChoiceDialog.makeChoice(
      "Do You Want to Restore this backup? Once started this process cannot be stopped!",
      "Yes",
      "No"
    );
    if (res == ChoiceDialog.CHOICE1) {
      ...
      Screen
        ...
        .addTab(
          ...
          () -> {
            Screen.setStatus(
              "Backup is still running in background!",
              10,
      ...
      LinkedList<File> files = new LinkedList<>();
      File backupChannel = new File(
        BACKUP_DIR,
        new File(DataManager.getWorkspace()).getName() +
        File.separator +
        Screen.getFileView().getProjectName() +
        File.separator +
        backupTitle
      ...
      for (File file : files) {
        ...
        try {
          ...
          while (i.available() > 0) o.write(i.read());
          o.close();
          i.close();
        } catch (Exception e) {
      ...
      printArea.print(
        "If the Files are already opened in the editors then, reload them."
      );
      printArea.print("-----------------------------------------");
      printArea.print(
        "All Backups are located at " + (new File(BACKUP_DIR).getAbsolutePath())
      );
      printArea.print(
        "You can create full project backups and also you can delete or modify backups from there."
      );
    }
  }
  ...
  public void loadAllFiles(LinkedList<File> files, File dir) {
    ...
    for (File f : F) {
      if (f.isDirectory()) loadAllFiles(files, f); else files.add(f);
    }
  }
  ...
  public void addToBackup(String backupTitle, File file) {
    ...
    if (!path.startsWith(Screen.getFileView().getProjectPath())) {
      System.err.println(
        "Cannot backup files that are located outside the project"
      );
      return;
    }
    String backupPath = path.substring(
      path.indexOf(Screen.getFileView().getProjectPath()) +
      Screen.getFileView().getProjectPath().length()
    );
