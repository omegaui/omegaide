...
public class DataManager extends DataBase {
  ...
  private static volatile boolean realTimeContentAssist = false;
  private static volatile boolean contentModeJava = true;
  private static volatile boolean useStarImports = false;
  private static volatile boolean sourceDefenderEnabled;
  private static volatile boolean parsingEnabled = true;
  ...
  private void loadData() {
    try {
      ...
      if (e == null) return;
      if (new File(e.getValue()).exists()) setDefaultProjectPath(e.getValue());
      setContentAssistRealTime(
        getEntryAt("Content Assist Real-Time", 0).getValueAsBoolean()
      );
      setUseStarImports(getEntryAt("Use Star Imports", 0).getValueAsBoolean());
      setPathToJava(
        getEntryAt(
          "Folder Containing Java Development Kits and Environments",
          0
        )
          .getValue()
      );
      ...
      setContentModeJava(
        getEntryAt("Content Mode Java", 0).getValueAsBoolean()
      );
      setSourceDefenderEnabled(
        getEntryAt("Source Defender Enabled", 0).getValueAsBoolean()
      );
      setConsoleCommand(
        getEntryAt("System Console Launch Command", 0).getValue()
      );
