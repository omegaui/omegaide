...
public class StructureView extends JDialog {
  ...
  public void init() {
    iconComp =
      new TextComp(
        IconManager.fluentstructureImage,
        25,
        25,
        back1,
        back1,
    ...
    fullLogComp =
      new TextComp(
        ...
        25,
        25,
        "Deassemble Headlessly (Uses System's Default javap command)",
        TOOLMENU_COLOR1_SHADE,
        back1,
        TOOLMENU_COLOR1,
        () -> {
    ...
    titleComp =
      new TextComp("View Java Code Structures", back1, back1, glow, null);
    ...
    searchScrollPanel.setBounds(
      5,
      5,
      searchPanel.getWidth() - 10,
      searchPanel.getHeight() - 10
    );
    ...
    contentScrollPanel.setBounds(
      5,
      30,
      primeContainerPanel.getWidth() - 10,
      primeContainerPanel.getHeight() - 35
    );
  ...
  public synchronized void search(String text) {
    ...
    for (Import im : JDKManager.imports) {
      if (im.getImport().contains(text)) matches.add(im);
    }
    if (matches.isEmpty()) {
    ...
    containerPanel.setPreferredSize(
      new Dimension(searchScrollPanel.getWidth(), block)
    );
  ...
  public void genView(Import im) {
    ...
    if (deassembleType == DEASSEMBLE_DYNAMICALLY) {
      ...
      if (CodeFramework.isSource(im.getImport())) {
        titleComp.setText(
          "Viewing " + im.getImport() + " (Dynamic, Type : SourceCode)"
        );
        SourceReader reader = new SourceReader(
          CodeFramework.getContent(im.getImport())
        );
        for (DataMember dx : reader.dataMembers) {
          TextComp comp = new TextComp(
            dx.getRepresentableValue(),
            back1,
            back1,
            dx.isMethod() ? TOOLMENU_COLOR5 : TOOLMENU_COLOR2,
            null
          );
        ...
        if (Assembly.has(im.getImport())) reader =
          Assembly.getReader(im.getImport()); else reader =
          Screen.getFileView().getJDKManager().prepareReader(im.getImport());

        if (reader.dataMembers == null) return;
      ...
      try {
        ...
        Process process = new ProcessBuilder(options)
    ...
    if (deassembleType == DEASSEMBLE_DYNAMICALLY) {
      contentPanel.setPreferredSize(
        new Dimension(contentScrollPanel.getWidth(), block)
      );
  ...
  public static Color getSuitableColor(Import im) {
    return im.getImport().startsWith("java")
      ? TOOLMENU_COLOR1
      : TOOLMENU_COLOR3;
  }
}
