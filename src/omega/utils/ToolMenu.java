...
public class ToolMenu extends JPanel {
    ...
    addMouseMotionListener(
      new MouseAdapter() {
        ...
        public void mouseDragged(MouseEvent e) {
          if (!maximized) screen.setLocation(
            e.getXOnScreen() - pressX,
            e.getYOnScreen() - pressY
          );
        }
      }
    );
  ...
  private void init() {
      ...
      image =
        ImageIO.read(
          getClass()
            .getResourceAsStream(
              "/omega_ide_icon128" +
              (UIManager.isDarkMode() ? "_dark.png" : ".png")
            )
        );
    } catch (Exception e) {
    ...
    closeComp =
      new TextComp("", c1, back2, c3, this::disposeAll) {
        @Override
        public void draw(Graphics2D g) {
          ...
          if (isMouseEntered()) {
            ...
            g.drawString(
              "x",
              getWidth() / 2 - g.getFontMetrics().stringWidth("x") / 2,
              getHeight() /
              2 -
    ...
    maximizeComp =
      new TextComp("", c1, back2, c3, this::maximize) {
        @Override
        public void draw(Graphics2D g) {
          g.setColor(back2);
          g.fillRect(0, 0, getWidth(), getHeight());
          ...
          if (isMouseEntered()) {
            ...
            g.drawString(
              maximized ? "><" : "<>",
              getWidth() / 2 - g.getFontMetrics().stringWidth("<>") / 2,
              getHeight() /
              2 -
              g.getFontMetrics().getHeight() /
              2 +
    ...
    minimizeComp =
      new TextComp("", c1, back2, c3, this::minimize) {
        @Override
        public void draw(Graphics2D g) {
          g.setColor(back2);
          g.fillRect(0, 0, getWidth(), getHeight());
    ...
    projectPopup =
      OPopupWindow.gen("Project Menu", screen, 0, false).width(250);
    initProjectPopup();
    ...
    taskMenu =
      new LabelMenu(
        "Click to Run Garbage Collector",
        () -> {
    ...
    runComp =
      new TextComp(
        ...
        20,
        20,
        "Run Project, Right Click to launch without build! (Not for Gradle)",
    ...
    runComp.addMouseListener(
      new MouseAdapter() {
        @Override
        public void mousePressed(MouseEvent e) {
          if (e.getButton() == 3) {
            if (GradleProcessManager.isGradleProject()) return;
            if (runComp.isClickable() && buildComp.isClickable()) Screen
              .getRunView()
              .justRun();
          }
        }
    ...
    instantRunComp =
      new TextComp(
        ...
        20,
        20,
        "Instant Run(Java Only), Uses System Default JDK for Building Project",
        TOOLMENU_COLOR2_SHADE,
        back3,
        TOOLMENU_COLOR2,
        () -> {
    ...
    instantBuildComp =
      new TextComp(
        fluentrocketbuildImage,
        20,
        20,
        "Instant Build(Java Only), Uses System Default JDK for Building Project",
        TOOLMENU_COLOR2_SHADE,
        back3,
        TOOLMENU_COLOR2,
        () -> {
    ...
    contentComp =
      new TextComp(
        ...
        TOOLMENU_COLOR1,
        () -> {
          DataManager.setContentAssistRealTime(
            !DataManager.isContentAssistRealTime()
          );
          contentComp.setToolTipText(
            DataManager.isContentAssistRealTime()
              ? "Content Assist is ON"
              : "Content Assist is Stopped"
          );
          contentComp.repaint();
        }
      ) {
    ...
    contentModeComp =
      new TextComp(
        ...
        () -> {
          ...
          contentModeComp.setToolTipText(
            DataManager.isContentModeJava()
              ? "Content Assist Mode : Java"
              : "Content Assist Mode : Tokenizer"
          );
    ...
    asteriskComp =
      new TextComp(
        ...
        () -> {
          ...
          asteriskComp.setToolTipText(
            DataManager.isUsingStarImports()
              ? "Using Asterisk Imports"
              : "Using Named Imports"
          );
    ...
    structureComp.setToolTipText(
      hidden ? "Project Structure Hidden" : "Project Structure Visible"
    );
    ...
    operateComp =
      new TextComp(
        ...
        () -> {
          ...
          operateComp.setToolTipText(
            oPHidden ? "Operation Panel Hidden" : "Operation Panel Visible"
          );
          operateComp.repaint();
        }
      ) {
    ...
    OPopupWindow consoleItemWindow = new OPopupWindow(
      "Select Console Type",
      screen,
      0,
      false
    ...
    consoleItemWindow
      .createItem(
        ...
        consoleSelector::launchTerminal
      )
      .createItem(
        "Faster Terminal",
        IconManager.fluentconsoleImage,
        Screen.getTerminalComp()::showTerminal
      )
      .createItem(
        "Full-Fledge Terminal",
        IconManager.fluentconsoleImage,
            ...
            NotificationPopup
              ...
              .iconButton(
                ...
                () -> {
                  ...
                  new Thread(() -> {
                    try {
                      if (Screen.onWindows()) new ProcessBuilder(
                        "java",
                        "-jar",
                        "Omega IDE.jar"
                      )
                        .start(); else new ProcessBuilder("omega-ide").start();
                    } catch (Exception e) {
                      e.printStackTrace();
                    }
                  })
    ...
    putAnimationLayer(
      shellComp,
      getImageSizeAnimationLayer(25, -8, false),
      ACTION_MOUSE_ENTERED
    );
    putAnimationLayer(
      structureViewComp,
    ...
    ImageSizeTransitionAnimationLayer layer = (ImageSizeTransitionAnimationLayer) getImageSizeAnimationLayer(
      25,
      -5,
      true
    );
    ...
    Screen
      ...
      .triggerReaction(
        PluginReactionEvent.genNewInstance(
          PluginReactionEvent.EVENT_TYPE_IDE_MINIMIZED,
          this,
          true
        )
      );
  ...
  public void maximize() {
    ...
    Screen
      ...
      .triggerReaction(
        PluginReactionEvent.genNewInstance(
          maximized
            ? PluginReactionEvent.EVENT_TYPE_IDE_MAXIMIZED
            : PluginReactionEvent.EVENT_TYPE_IDE_RESTORED,
          this,
          maximized
        )
      );
  ...
  public void reloadItems(boolean non_java) {
    ...
    if (Screen.getFileView().getJDKManager() != null) jdkItem.setName(
      "Project JDK : Java " +
      Screen.getFileView().getJDKManager().getVersionAsInt()
    ); else jdkItem.setName("Project JDK : None");

    if (
      DataManager.getInstantMode().equals(DataManager.INSTANT_MODE_SPEED)
    ) instantModeItem.setName("Instant Mode : Speed"); else if (
      DataManager.getInstantMode().equals(DataManager.INSTANT_MODE_ACCURACY)
    ) instantModeItem.setName("Instant Mode : Accuracy");

    parsingEnabledItem.setName(
      "Parsing Enabled : " + DataManager.isParsingEnabled()
    );
  }
  ...
  public void changeLang() {
    ...
    langComp.image =
      LanguageTagView.getRespectiveTagImage(
        Screen.getFileView().getProjectManager().getLanguageTag()
      );
    langComp.repaint();
  }
  ...
  private void initSetMenu() {
    ...
    setPopup
      .createItem(
        "Change Font",
        IconManager.settingsImage,
        () -> {
          Font font = fontC.chooseFont(
            new Font(
              UIManager.fontName,
              UIManager.fontState,
              UIManager.fontSize
      ...
      .createItem(
        "Change Workspace",
        IconManager.settingsImage,
        () -> new omega.utils.WorkspaceSelector(screen).setVisible(true)
      )
      .createItem(
        "Animations",
        IconManager.settingsImage,
    ...
    typeItem.setAction(() -> {
      omega.Screen.getFileView().getProjectManager().non_java =
        !omega.Screen.getFileView().getProjectManager().non_java;
      typeItem.setName(
        omega.Screen.getFileView().getProjectManager().non_java
          ? "Project Type : Other/Gradle"
          : "Project Type : Java"
      );
      omega.Screen
        .getScreen()
        .manageTools(omega.Screen.getFileView().getProjectManager());
    ...
    if (
      DataManager.getInstantMode().equals(DataManager.INSTANT_MODE_SPEED)
    ) text = "Instant Mode : Speed"; else if (
      DataManager.getInstantMode().equals(DataManager.INSTANT_MODE_ACCURACY)
    ...
    instantModeItem =
      new OPopupItem(
        setPopup,
        text,
        IconManager.fluentrocketImage,
        () -> {
          DataManager.setInstantMode(
            (
                DataManager
                  .getInstantMode()
                  .equals(DataManager.INSTANT_MODE_SPEED)
              )
              ? DataManager.INSTANT_MODE_ACCURACY
              : DataManager.INSTANT_MODE_SPEED
          );
          ...
          if (
            ...
            DataManager
              .getInstantMode()
              .equals(DataManager.INSTANT_MODE_ACCURACY)
          ) instantModeItem.setName("Instant Mode : Accuracy");
        }
      );
        ...
        () -> {
          if (GradleProcessManager.isGradleProject()) {
            Screen
              ...
              .loadFile(
                new File(
                  Screen.getFileView().getProjectPath(),
                  "settings.gradle"
                )
              );
            return;
          }
          if (Screen.getFileView().getProjectManager().non_java) Screen
            .getUniversalSettingsView()
            .setVisible(true);
        }
      );
  ...
  private void initHelpMenu() {
    helpPopup
      .createItem(
        "Stucked? See Tutorial Videos",
        IconManager.fluentyoutubeImage,
        () -> {
          try {
            java.awt.Desktop
              .getDesktop()
      ...
      .createItem(
        "Plugin Store",
        IconManager.ideImage64,
        () -> Screen.getPluginStore().setVisible(true)
      )
      .createItem(
        "Plugin Manager",
        IconManager.ideImage64,
        () -> Screen.getPluginsView().setVisible(true)
      )
      .createItem(
        "Check for Update",
        IconManager.ideImage64,
  ...
  private void initToolMenu() {
    toolsPopup
      .createItem(
        "Source Defender",
        IconManager.fluentsourceImage,
        () -> sourceDefender.setVisible(true)
      )
      .createItem(
        "Process Wizard",
        IconManager.fluentbuildImage,
        () -> processWizard.setVisible(true)
      )
      .createItem(
        "Snippet Manager",
        IconManager.buildImage,
        () -> Screen.snippetView.setVisible(true)
      )
      .createItem(
        "Generate Getter/Setter",
        IconManager.buildImage,
        () -> omega.gset.Generator.gsView.genView(screen.getCurrentEditor())
      )
      .createItem(
        "Override/Implement Methods",
        IconManager.buildImage,
        () -> omega.gset.Generator.overView.genView(screen.getCurrentEditor())
      )
      .createItem(
        "Color Picker",
        IconManager.fluentcolorwheelImage,
  ...
  private void initProjectPopup() {
    ...
    projectPopup
      ...
      .createItem(
        "Initialize Gradle",
        IconManager.fluentgradleImage,
        GradleProcessManager::init
      )
      .createItem(
        "Create Gradle Module",
        IconManager.fluentgradleImage,
        () -> ToolMenu.gradleModuleWizard.setVisible(true)
      )
      .createItem(
        "Delete Project",
        IconManager.fluentdemonImage,
        () -> projectDistructionWizard.setVisible(true)
      );
  }
  ...
  private void initFilePopup() {
    ...
    filePopup
      ...
      .createItem(
        "New Project (non-java project)",
        IconManager.projectImage,
        () -> universalProjectWizard.setVisible(true)
      );
    recentFilePopup =
      OPopupWindow
        .gen("Recent Files Menu", screen, 0, true)
        .width(350)
        .height(250);
    fileMenu =
      new OPopupItem(
    ...
    recentProjectPopup =
      OPopupWindow
        .gen("Recent Projects Menu", screen, 0, true)
        .width(350)
        .height(250);
    projectMenu =
      new OPopupItem(
    ...
    allProjectsPopup =
      OPopupWindow
        .gen("All Projects Menu", screen, 0, true)
        .width(350)
        .height(250);
    allMenu =
      new OPopupItem(
    ...
    filePopup
      ...
      .createItem(
        "Save Everything and Exit -Without terminating running apps",
        IconManager.closeImage,
        () -> {
          Screen.notify("Saving UI and Data");
          Screen.getPluginManager().save();
          screen.getUIManager().save();
      ...
      .createItem(
        "Terminate Running Apps and Exit -Without saving opened editors",
        IconManager.closeImage,
        () -> {
          Screen.notify("Terminating Running Applications");
          try {
  ...
  public class LabelMenu extends JComponent {
      ...
      addMouseListener(
        new MouseAdapter() {
          ...
          public void mouseExited(MouseEvent e) {
            enter = false;
            if (!CodeFramework.resolving) setTask(
              "Hover to see Memory Statistics"
            );
            repaint();
          }
    ...
    public void setText(String text) {
      if (this.msg == null) this.text = text;
      repaint();
    }

    public void setMsg(String msg) {
      this.msg = msg;
      if (msg != null) this.text = msg; else this.text =
        "Hover to see Memory Statistics";
      repaint();
    }
      ...
      addMouseListener(
        new MouseAdapter() {
          ...
          public void mousePressed(MouseEvent e) {
            popup.setLocation(
              getX() + screen.getX(),
              getY() + getHeight() + 15 + getHeight() + screen.getY()
            );
