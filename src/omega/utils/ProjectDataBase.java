...
public class ProjectDataBase extends DataBase {
  ...
  public ProjectDataBase() {
    super(
      Screen.getFileView().getProjectPath() + File.separator + ".projectInfo"
    );
    load();
  }
  ...
  public void load() {
    jdkPath =
      getEntryAt("JDK Path", 0) != null
        ? getEntryAt("JDK Path", 0).getValue()
        : null;
    mainClass =
      getEntryAt("Main Class", 0) != null
        ? getEntryAt("Main Class", 0).getValue()
        : "";
    non_java =
      getEntryAt("Non-Java Project", 0) != null
        ? getEntryAt("Non-Java Project", 0).getValueAsBoolean()
        : false;
    if (!non_java) {
    ...
    LinkedList<DataEntry> mainEditors = getEntries(
      "Opened Editors on Main Tab Panel"
    );
    LinkedList<DataEntry> rightEditors = getEntries(
      "Opened Editors on Right Tab Panel"
    );
    LinkedList<DataEntry> bottomEditors = getEntries(
      "Opened Editors on Bottom Tab Panel"
    );
    LinkedList<DataEntry> jars = getEntries(
      "Project Classpath : Required Jars"
    );
    LinkedList<DataEntry> natives = getEntries(
      "Project Classpath : Required Native Libraries"
    );
    LinkedList<DataEntry> resourceRoots = getEntries(
      "Project Classpath : Required Resource Roots"
    );
    LinkedList<DataEntry> modules = getEntries(
      "Project Classpath : Required Modules"
    );
    ...
    if (getEntryAt("Language Tag", 0) != null) setLanguageTag(
      getEntryAt("Language Tag", 0).getValueAsInt()
    );
    ...
    if (mainEditors != null) {
      for (DataEntry e : mainEditors) {
        ...
        if (f.exists()) Screen.getFileView().getScreen().loadFile(f);
      }
    }
    ...
    if (jars != null) {
      for (DataEntry e : jars) {
        ...
        value = modifyProjectPath(value);
        File f = new File(value);
        if (f.exists()) this.jars.add(value);
      }
    }
    if (natives != null) {
      for (DataEntry e : natives) {
        ...
        value = modifyProjectPath(value);
        File f = new File(value);
        if (f.exists()) this.natives.add(value);
      }
    }
    if (resourceRoots != null) {
      for (DataEntry e : resourceRoots) {
        ...
        value = modifyProjectPath(value);
        File f = new File(value);
        if (f.exists()) this.resourceRoots.add(value);
      }
    }
    if (modules != null) {
      for (DataEntry e : modules) {
        ...
        value = modifyProjectPath(value);
        File f = new File(value);
        if (f.exists()) this.modules.add(value);
      }
    }
  ...
  public void save() {
    ...
    addEntry(
      "Main Class",
      Screen.getRunView().mainClass != null ? Screen.getRunView().mainClass : ""
    );
  ...
  public String modifyProjectPath(String value) {
    if (!value.startsWith(PROJECT_ROOT)) return value;
    return (
      getProjectPath() +
      value.substring(value.indexOf(PROJECT_ROOT) + PROJECT_ROOT.length())
    );
  }

  public String genProjectRootPath(String value) {
    if (!value.startsWith(getProjectPath())) return value;
    return (
      PROJECT_ROOT +
      value.substring(
        value.indexOf(getProjectPath()) + getProjectPath().length()
      )
    );
  }
  ...
  public static void genInfo(String projectPath, boolean non_java) {
    try {
      ...
      writer.println(
        ...
        (
          non_java
            ? LanguageTagView.LANGUAGE_TAG_ANY
            : LanguageTagView.LANGUAGE_TAG_JAVA
        )
      );
      writer.close();
    } catch (Exception e) {
