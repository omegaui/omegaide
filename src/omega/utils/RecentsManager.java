...
public class RecentsManager {
  ...
  public static final LinkedList<String> RECENTS = new LinkedList<>();
  ...
  public void loadData() {
    ...
    try {
      ...
      RECENTS.forEach(r -> {
        ...
        if (fileX.isFile()) {
          ToolMenu.recentFilePopup.createItem(
      ...
      RECENTS.forEach(r -> {
        ...
        if (fileX.isDirectory()) {
          ToolMenu.recentProjectPopup.createItem(
            r.substring(r.lastIndexOf(File.separator) + 1),
      ...
      if (home.exists()) {
        ...
        for (File fileZ : files) {
          if (fileZ.isDirectory()) {
            ToolMenu.allProjectsPopup.createItem(
              fileZ.getName(),
              IconManager.projectImage,
              () -> {
                ToolMenu.recentProjectPopup.setVisible(false);
  ...
  public static synchronized void add(String path) {
    if (RECENTS.indexOf(path) < 0) {
      ...
      if (RECENTS.size() > 20) {
        if (file.isDirectory()) {
          if (getNumberOfProjects() > 10) removeFirstProject(); else if (
            getNumberOfFiles() > 10
          ) removeFirstFile();
        }
      }
  ...
  public static void removeAllProjects() {
    ...
    for (String path : RECENTS) {
      file = new File(path);
      if (file.isDirectory()) paths.add(path);
    }
