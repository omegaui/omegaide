...
public class BuildPathManager extends JDialog {
  ...
  public void init() {
    ...
    jarPanel =
      new BuildPanel("Click \"Add\" to Add Jar Files(.jar) to Classpath");
    nativePanel =
      new BuildPanel("Click \"Add\" to Add Native Library Roots to Classpath");
    resourceRootPanel =
      new BuildPanel("Click \"Add\" to Add Resource Roots to Classpath");
    modulePanel =
      new BuildPanel(
        "Click \"Add\" to Add Module Files(.jar modular) to Modulepath"
      );
      ...
      if (!files.isEmpty()) {
        for (File file : files) {
          Screen
            .getFileView()
            .getProjectManager()
            .natives.add(file.getAbsolutePath());
        }
      }
    } else if (state == 2) {
      fs.setFileExtensions(FileSelectionDialog.ALL_EXTENSIONS);
      ...
      if (!files.isEmpty()) {
        for (File file : files) {
          Screen
            .getFileView()
            .getProjectManager()
            .resourceRoots.add(file.getAbsolutePath());
        }
      }
    } else {
      fs.setFileExtensions(".jar");
      ...
      if (!files.isEmpty()) {
        for (File file : files) {
          Screen
            .getFileView()
            .getProjectManager()
            .modules.add(file.getAbsolutePath());
        }
      }
    }
    read();
  ...
  public void removePath() {
    jarComps.forEach(comp -> {
      if (comp.color2 == TOOLMENU_COLOR3) Screen
        .getFileView()
        .getProjectManager()
        .jars.remove(comp.getToolTipText());
    });
    nativeComps.forEach(comp -> {
      if (comp.color2 == TOOLMENU_COLOR3) Screen
        .getFileView()
        .getProjectManager()
        .natives.remove(comp.getToolTipText());
    });
    resourceRootComps.forEach(comp -> {
      if (comp.color2 == TOOLMENU_COLOR3) Screen
        .getFileView()
        .getProjectManager()
        .resourceRoots.remove(comp.getToolTipText());
    });
    moduleComps.forEach(comp -> {
      if (comp.color2 == TOOLMENU_COLOR3) Screen
        .getFileView()
        .getProjectManager()
        .modules.remove(comp.getToolTipText());
    });
    read();
  }
  ...
  public String getModulePath() {
    LinkedList<String> modules = Screen
      .getFileView()
      .getProjectManager()
      .modules;
    ...
    modules.forEach(modulePath -> {
      String parentPath = modulePath.substring(
        0,
        modulePath.lastIndexOf(File.separatorChar)
      );
      ...
      if (!contains) parentDirs.add(parentPath);
    });

    for (String p : parentDirs) path += p + Screen.PATH_SEPARATOR;
