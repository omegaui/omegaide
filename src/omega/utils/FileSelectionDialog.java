...
public class FileSelectionDialog extends JDialog {
  ...
  public void init() {
    ...
    levelComp =
      new TextComp(
        IconManager.fluentlevelupImage,
        20,
        20,
        "Move One Level Up",
        ...
        () -> {
          ...
          if (File.pathSeparator.equals(":") && !path.equals("/")) {
            if (count('/', path) != 1) currentDir =
              new File(
                path.substring(0, path.lastIndexOf('/'))
              ); else currentDir = new File("/");
          ...
          if (state == 0) selectFiles(); else if (
            state == 1
          ) selectDirectories(); else if (
            state == 2
    ...
    selectionField =
      new NoCaretField(
        "",
        "or Enter the path manually and hit enter",
        TOOLMENU_COLOR2,
        back2,
        TOOLMENU_COLOR3
      );
      ...
      if (file.exists()) {
        if (state == 0) {
          ...
          if (file.isDirectory()) {
            selections.add(file);
            dispose();
          } else selectionField.notify("Directory Expected!");
        } else if (state == 2) {
          selections.add(file);
          dispose();
        }
  ...
  public boolean isExtentionAllowed(File file) {
    if (
      extensions == null ||
      extensions.length == 0 ||
      extensions[0].equals(ALL_EXTENSIONS)
    ) return true;
    if (file.isDirectory() && isDirectoriesAllowed()) return true;
  ...
  public LinkedList<File> showWindowsRoots() {
    ...
    if (F == null || F.length == 0) return selections;
    ...
    for (File file : files) {
      if (isExtentionAllowed(file)) {
        ...
        String meta = (file.listFiles() != null && file.listFiles().length != 0)
          ? ""
          : " - Empty";
        if (!file.isDirectory()) meta = "";
        ToggleComp comp = new ToggleComp(
          FileTreeBranch.getPreferredImageForFile(file),
          25,
          25,
          file.toString() + meta,
          c1,
        ...
        if (!file.isDirectory()) {
          comp.setOnToggle(value -> {
            ...
            if (value) selections.add(file); else selections.remove(file);
          });
        } else comp.toggleEnabled = false;
  ...
  public static synchronized void sort(LinkedList<File> files) {
    try {
      ...
      files.forEach(f -> {
        if (f.isDirectory()) tempDirs.add(f); else tempFiles.add(f);
      });
      files.clear();
      File[] F = new File[tempFiles.size()];
      int k = -1;
      for (File fx : tempFiles) F[++k] = fx;
      File[] D = new File[tempDirs.size()];
      k = -1;
      for (File fx : tempDirs) D[++k] = fx;
      sort(F);
      sort(D);
      LinkedList<File> dots = new LinkedList<>();
      for (File f : D) {
        if (f.getName().startsWith(".")) dots.add(f); else files.add(f);
      }
      for (File f : dots) {
        files.add(f);
      }
