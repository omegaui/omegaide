...
public class ComponentResizer extends MouseAdapter {
  ...
  private static final Dimension MAXIMUM_SIZE = new Dimension(
    Integer.MAX_VALUE,
    Integer.MAX_VALUE
  );
  ...
  public void mouseMoved(MouseEvent e) {
    ...
    if (location.x > source.getWidth() - dragInsets.right - 1) direction +=
      EAST;

    if (location.y < dragInsets.top) direction += NORTH;

    if (location.y > source.getHeight() - dragInsets.bottom - 1) direction +=
      SOUTH;
  ...
  protected void changeBounds(
    ...
    if (WEST == (direction & WEST)) {
      ...
      drag =
        getDragBounded(drag, snapSize.width, width, minimumSize.width, maximum);
